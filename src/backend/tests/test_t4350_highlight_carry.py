...
class TestFirstExportSeed:
    ...
    def test_seeded_but_unplaced_prior_reseeds_from_detection(self, video_dims, simple_crop_keyframes, detected_regions_sentinel):
        """T11000 (prod, 2026-09-21): the prior working video's only region was the
        detection seed with `keyframes: []` (the user never clicked a player), and
        the re-export changed framing (slow-mo added). The keyframe-less region
        cannot be transformed (to_raw returns None), so before the fix the carry
        returned zero regions + 'dropped:1' and threw away the detection that had
        just run. A prior with no placed spotlight is a first export: seed."""
        ...
        seeded_prior = [{
            'id': 'region-auto-0-0', 'start_time': 0.0, 'end_time': 2.0, 'enabled': True,
            'autoGenerated': True, 'keyframes': [],
            'detections': [{'timestamp': 0.0, 'frame': 0, 'boxes': [{'x': 1, 'y': 1, 'width': 2, 'height': 2}]}],
        }]

        result, note = resolve_carried_highlights(
            prior_highlights=seeded_prior,
    ...
    def test_placed_region_still_carries_not_reseeds(self, video_dims, simple_crop_keyframes, detected_regions_sentinel):
        """Guard for T4350 Q5: once ANY prior region has a keyframe, detection must
        not overwrite it -- the transform path runs, even alongside an unplaced seed."""
        old_segments = {'boundaries': [0.0, 15.0], 'segmentSpeeds': {}, 'trimRange': None}
        new_segments = {'boundaries': [0.0, 15.0], 'segmentSpeeds': {}, 'trimRange': {'start': 1.0, 'end': 15.0}}
        old_snapshot = _snapshot(1, video_dims, [_clip_entry(old_segments, simple_crop_keyframes)])
        new_snapshot = _snapshot(1, video_dims, [_clip_entry(new_segments, simple_crop_keyframes)])
        prior = [
            {'id': 'seed', 'start_time': 0.0, 'end_time': 2.0, 'enabled': True, 'autoGenerated': True, 'keyframes': []},
            _region('placed', 5.0, 7.0, kf_time=5.0),
        ]

        result, _note = resolve_carried_highlights(
            prior_highlights=prior,
            prior_snapshot=old_snapshot,
            new_snapshot=new_snapshot,
            detected_regions=detected_regions_sentinel,
            clip_count=1,
        ...
        assert [r['id'] for r in result] == ['placed']
        assert result != detected_regions_sentinel
