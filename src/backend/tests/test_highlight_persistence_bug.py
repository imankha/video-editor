...
class TestAPICodePath:
    ...
    This simulates what happens in _load_highlights_from_raw_clips.
    """

    def test_simulate_load_highlights_from_raw_clips(self):
        """
        Simulate the _load_highlights_from_raw_clips function with actual data.

        This tests the exact query and logic used in overlay.py.
        """
        import sqlite3
        from pathlib import Path

        # Connect to user A's actual database
        ...
        # This is the exact query from _load_highlights_from_raw_clips
        cursor.execute("""
        ...
        print(f"\nQuery returned {len(clips)} clips")

        for clip in clips:
            print(f"  Clip {clip['id']}: raw_clip_id={clip['raw_clip_id']}")
            print(f"    crop_data: {clip['crop_data'][:100] if clip['crop_data'] else 'None'}...")
            print(f"    segments_data: {clip['segments_data'][:100] if clip['segments_data'] else 'None'}...")
            print(f"    default_highlight_regions: {clip['default_highlight_regions'][:100] if clip['default_highlight_regions'] else 'None'}...")

        # Should find at least one clip with highlights
        assert len(clips) > 0, "No clips with default_highlight_regions found for project 1"
    ...
    def test_full_transformation_with_db_data(self):
        """
        Load actual data from database and run the full transformation.
        This is the closest simulation of _load_highlights_from_raw_clips.
        """
        import sqlite3
        from pathlib import Path
        ...
        # Get working video info for dimensions
        # Note: in the actual code, this tries to read the video file
        # We'll use the default dimensions
        working_video_dims = {"width": 1080, "height": 607}
        ...
        for clip in clips:
            print(f"\nProcessing clip {clip['id']} (raw_clip_id={clip['raw_clip_id']})")
            ...
            if clip['default_highlight_regions']:
                try:
                    ...
                    print(f"  Loaded {len(raw_regions)} raw regions")
                except json.JSONDecodeError as e:
                    print(f"  Failed to parse: {e}")
                    continue

            if not raw_regions:
                print(f"  No raw regions")
                continue
            ...
            if clip['crop_data']:
                try:
                    ...
                    print(f"  Loaded {len(crop_keyframes)} crop keyframes")
                except json.JSONDecodeError:
                    pass
            ...
            if clip['segments_data']:
                try:
                    ...
                    print(f"  Loaded segments_data: {segments_data}")
                except json.JSONDecodeError:
                    pass

            # Transform regions
            print(f"  Calling transform_all_regions_to_working...")
            ...
            print(f"  Got {len(transformed_regions)} transformed regions")

            all_transformed_regions.extend(transformed_regions)

        print(f"\nTotal transformed regions: {len(all_transformed_regions)}")

        assert len(all_transformed_regions) > 0, "No regions were transformed!"

        # Verify the first region has valid keyframes
        if all_transformed_regions:
            region = all_transformed_regions[0]
            print(f"\nFirst region: {json.dumps(region, indent=2)}")
            assert 'keyframes' in region
            assert len(region['keyframes']) > 0
