"""T8120 (moved by T11170): upfront welcome-credit grant + idempotency.

Per-quest credit rewards are retired; the whole welcome total
(storage_credits.WELCOME_CREDITS) is granted upfront through
storage_credits.grant_welcome_credits — at signup for new users, as the ungranted
remainder on next login for existing accounts. T11170 moved this grant off the
quest system (it used to be credit_ledger.grant_quest_chain_credits, reading
quest_config.QUEST_CHAIN_CREDIT_TOTAL). It must NEVER double-grant: a repeat call
is a no-op, and a user who already claimed some legacy per-quest rewards gets only
the remainder.

Real Postgres via the `pg_conn` fixture (gate open by default).
"""

from app.services import credit_ledger
from app.services.credit_ledger import (
    credit_key,
    get_balance,
    grant,
    has_key,
)
from app.services.storage_credits import WELCOME_CREDITS, grant_welcome_credits

USER = "user-a"


class TestUpfrontGrant:
    def test_new_user_gets_full_welcome_total(self, pg_conn):
        result = grant_welcome_credits(USER)
        assert result["applied"] is True
        assert result["granted"] == WELCOME_CREDITS
        assert get_balance(USER) == WELCOME_CREDITS
        # The fixed per-user idempotency key is recorded.
        assert has_key(USER, credit_key("quest_upfront", USER))

    def test_second_call_is_a_noop_no_double_grant(self, pg_conn):
        first = grant_welcome_credits(USER)
        second = grant_welcome_credits(USER)

        assert first["granted"] == WELCOME_CREDITS
        assert second["applied"] is False
        assert second["granted"] == 0
        assert get_balance(USER) == WELCOME_CREDITS, "repeat login must not double-grant"

    def test_mid_quest_user_gets_only_the_remainder(self, pg_conn):
        # Simulate an existing account that already claimed quest_1 under the
        # legacy per-quest path (source='quest_reward'): 15 credits.
        grant(USER, 15, "quest_reward", credit_key("quest_reward", "quest_1"), reference_id="quest_1")
        assert get_balance(USER) == 15

        result = grant_welcome_credits(USER)
        assert result["applied"] is True
        # Remainder = total - already-claimed, never the full total again.
        assert result["granted"] == WELCOME_CREDITS - 15
        assert get_balance(USER) == WELCOME_CREDITS

        # And it's still idempotent from there.
        again = grant_welcome_credits(USER)
        assert again["granted"] == 0
        assert get_balance(USER) == WELCOME_CREDITS

    def test_fully_claimed_legacy_user_gets_nothing(self, pg_conn):
        # A user who already claimed every quest under the legacy path (sum ==
        # the welcome total) gets a pure no-op and no upfront key is written.
        grant(USER, WELCOME_CREDITS, "quest_reward",
              credit_key("quest_reward", "quest_all"), reference_id="quest_all")

        result = grant_welcome_credits(USER)
        assert result["applied"] is False
        assert result["granted"] == 0
        assert get_balance(USER) == WELCOME_CREDITS
        assert not has_key(USER, credit_key("quest_upfront", USER))

    def test_coexists_with_signup_bonus(self, pg_conn):
        # The upfront grant lives in a distinct key space from the new-account
        # bonus (signup:) so both land for a brand-new user.
        credit_ledger.grant_credits(USER, 8, source="new_account_bonus")
        grant_welcome_credits(USER)
        assert get_balance(USER) == 8 + WELCOME_CREDITS


class TestQuestPanelCollapsedPersistence:
    """T8120: the collapse-to-Help state persists per-user in user_settings KV
    (no schema change), so a collapse survives navigation AND reload."""

    def test_collapsed_flag_round_trips(self):
        from app.services.user_db import (
            ensure_user_database,
            get_quest_panel_collapsed,
            set_quest_panel_collapsed,
        )

        uid = "quest-collapse-user"
        ensure_user_database(uid)

        # Default (no key) is expanded.
        assert get_quest_panel_collapsed(uid) is False
        # Collapse persists.
        set_quest_panel_collapsed(uid, True)
        assert get_quest_panel_collapsed(uid) is True
        # And it can be re-expanded.
        set_quest_panel_collapsed(uid, False)
        assert get_quest_panel_collapsed(uid) is False
