...
def test_registry_head_is_v041():
    ...
    # it merged master and found v037 already past the v040 head.
    assert max(m.version for m in MIGRATIONS) == 41
    ...
    assert {34, 35, 36, 38, 40, 41} <= registered
    # v037 / v039 belong to the sibling T5215 (pre-renumber) / T6630 branches.
    # They must be renumbered ABOVE this head before they merge, or the runner
    # skips them. T5215 already did (-> v041, above); v039 (T6630) has not yet.
