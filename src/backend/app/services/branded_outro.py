...
def append_branded_outro(in_path: str, out_path: str) -> bool:
    ...
    Never raises: outro failure must not abort a (possibly paid) export.
    """
    ...
    tmp_dir = tempfile.mkdtemp(prefix="branded_outro_")
    card_path = os.path.join(tmp_dir, "card.mp4")
    try:
        info = _probe_media(in_path)
        _build_outro_card(card_path, info)
