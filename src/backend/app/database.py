...
# ---------------------------------------------------------------------------
# Explicit sync functions for background tasks (T940)
# ---------------------------------------------------------------------------

def sync_db_to_r2_explicit(user_id: str, profile_id: str) -> bool:
    """Sync profile database to R2. For use outside request middleware (e.g., background workers).

    Unlike sync_db_to_cloud(), this takes explicit user_id/profile_id instead of
    reading from request context vars (which may not be set in BackgroundTasks).
    """
    if not R2_ENABLED:
        return True
    ...
    if success and new_version is not None:
        set_local_db_version(user_id, profile_id, new_version)
        logger.debug(f"[ExplicitSync] Profile DB synced for user={user_id} profile={profile_id} v{new_version}")
        return True
    elif not success:
        logger.warning(f"[ExplicitSync] Profile DB sync failed for user={user_id} profile={profile_id}")
        return False
    return True


def sync_user_db_to_r2_explicit(user_id: str) -> bool:
    """Sync user.sqlite to R2. For use outside request middleware (e.g., background workers)."""
    if not R2_ENABLED:
        return True
    ...
    if success and new_version is not None:
        set_local_user_db_version(user_id, new_version)
        logger.debug(f"[ExplicitSync] user.sqlite synced for user={user_id} v{new_version}")
        return True
    elif not success:
        logger.warning(f"[ExplicitSync] user.sqlite sync failed for user={user_id}")
        return False
    return True
