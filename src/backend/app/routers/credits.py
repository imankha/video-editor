...
async def grant(request: GrantRequest):
    ...
    (T550)", but both grant server-side today and never call this endpoint:
      - quests    -> T8120: no longer per-quest; session_init.py grant_welcome_credits(user_id)
                     (T11170: storage_credits) grants the whole welcome total upfront at signup/next login
