 ...
 *
 * The sample MP4 is generated by ffmpeg in beforeAll (not committed), same as
 * T5450/T5860. It used to read src/landing/public/before_after_demo.mp4, but the
 * Astro landing rebuild (9c925c18) removed that file, so mockCollection's
 * readFileSync threw ENOENT and failed all 4 tests in ~200ms. A generated fixture
 * has no cross-package dependency to rot: this spec only needs SOME real mp4 whose
 * moov atom gives the <video> a true duration.
 *
...
const SAMPLE_MP4 = path.resolve(__dirname, '..', 'test-results', 'T5100-sample.mp4');
// Single source of truth for the fixture's length: fed to ffmpeg AND used as the
// "duration has settled" target below (which previously hardcoded 11.47 -- the old
// landing demo's length, a magic number that had to be rediscovered by hand).
const SAMPLE_DURATION_S = 6;

test.beforeAll(() => {
  if (!fs.existsSync(SAMPLE_MP4)) {
    fs.mkdirSync(path.dirname(SAMPLE_MP4), { recursive: true });
    execSync(
      `ffmpeg -y -f lavfi -i testsrc=duration=${SAMPLE_DURATION_S}:size=640x360:rate=30 ` +
        `-pix_fmt yuv420p -movflags +faststart "${SAMPLE_MP4}"`,
      { stdio: 'ignore' }
    );
  }
});
...
test.afterAll(() => {
  if (fs.existsSync(SAMPLE_MP4)) fs.unlinkSync(SAMPLE_MP4);
});
...
test.describe('T5100 compilation timeline seek + hover', () => {
  ...
  test('click a segment loads that reel and seeks to the clicked fraction (criteria 2 + 5)', async ({ page }) => {
    ...
    await page.waitForFunction((expected) => {
      const v = document.querySelector('video');
      return v && Math.abs(v.duration - expected) < 0.05; // settled final duration
    }, SAMPLE_DURATION_S, { timeout: 8000 });
