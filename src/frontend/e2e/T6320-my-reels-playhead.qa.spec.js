 ...
 * finally reaches Published' CollectionPlayer, and that the mechanical move to
 * the shared ProgressTrack/PlayheadHandle primitives changed nothing else.
 *
 * Criteria evidenced here:
 *   c1  Published shows the sport-ball playhead on the active segment (multi-reel
...
test.describe('T6320 — Published playhead handle (real account)', () => {
  ...
  test('c4: responsive sweep, 375px + desktop, no horizontal overflow', async ({ page }) => {
    ...
    await responsiveSweep(page, async () => {
      // Published drawer itself must not introduce horizontal scroll at either size.
    });
  });
});
