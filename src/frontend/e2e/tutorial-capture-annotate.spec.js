/**
 * Tutorial capture: ANNOTATE quest — home → pick sport → open game → scrub →
 * Add Clip → fill the clip editor → Save → Playback Annotations → Share Annotations.
 * Records 1920x1080 with guidance overlays + beacon marks (see helpers/tutorialCapture).
 * Marks map to line numbers in ReelBallersTutroials/annotate/talk_track.txt.
 *
 ...
 *     npx playwright test e2e/tutorial-capture-annotate.spec.js --reporter=line
 */
...
test('capture annotate tutorial footage', async ({ browser }) => {
  ...
  await page.getByRole('button', { name: 'Games', exact: false }).first().waitFor();
  await page.mouse.move(960, 400);
  await mark(0);                              // "start on the home screen"
  await dwell(3);
  ...
  await mark(1, 'sport');
  await act(sportBtn);
  await dwell(1.4);                           // Manage Profiles modal opens
  const profileRow = page.locator(
    'button[title="Active profile"], button[title="Switch to this profile"]').first();
  try { await act(profileRow); } catch { await page.keyboard.press('Escape'); }
  await clearRing();
  await dwell(1.5);
  await page.keyboard.press('Escape');        // make sure the modal is gone
  await dwell(0.5);
  ...
  await dwell(1);
  const gameCard = page.getByRole('button', { name: GAME }).first();
  await ring(gameCard, 8);
  await dwell(0.8);
  await mark(2, 'opens');
  await act(gameCard);
  await clearRing();
  const addClip = page.getByRole('button', { name: 'Add Clip' }).first();
  await addClip.waitFor({ timeout: 30000 });  // annotate view ready
  await videosReady(1, 20000);
  await dwell(1.5);
  ...
  try {
    ...
    const bar = page.locator('div.cursor-pointer.z-10').first();
  ...
  try {
    ...
    const g = await green.boundingBox();
    const y = g.y + g.height / 2;
    await drag(g.x + 3, y, g.x - 28, y, 20);            // start handle earlier
    await dwell(0.6);
    const g2 = await green.boundingBox();
    await drag(g2.x + g2.width - 3, y, g2.x + g2.width + 22, y, 20);  // end later
  } catch {}
  await dwell(1.6);
  ...
  try {
    await typeInto(page.getByPlaceholder('Add a note about this clip...'), 'Great vision to start the counter');
  } catch {}
  await dwell(0.8);
  ...
  try {
    const toggle = page.locator('xpath=//label[normalize-space()="My Athlete"]/following-sibling::button[1]').first();
    await ring(toggle, 8);                     // already ON by default — just point at it
  ...
  // --- line 12: Create Reel toggle ----------------------------------------------------------
  const createReel = page.getByRole('button', { name: 'Create Reel' }).first();
  try { await ring(createReel, 8); } catch {}
  await dwell(0.8);
  await mark(12, 'Create');
  try { await act(createReel); } catch {}
  await clearRing();
  await dwell(1.6);
  ...
  try {
    ...
    for (const p of projects.filter(p => p.name === CLIP_NAME)) {
