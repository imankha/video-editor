 ...
 *
 * Root cause (verified): FocusScreen subscribes to the WHOLE focusStore (selector-less
 * `useFocusStore()`), and the effect that exposes `saveCurrentClipState` to updateFlush.js
...
import { useFocusStore, useRegisterActiveSaveHandler } from './focusStore';

const RENDER_CAP = 60; // above React's 50 nested-update limit; our own guard so a loop can't hang

afterEach(() => {
  useFocusStore.setState({ activeSaveCurrentClipState: null });
});
...
function useReactiveRegister_PREFIX(handler) {
  useEffect(() => {
    useFocusStore.getState().registerSaveCurrentClipState(handler);
    return () => useFocusStore.getState().clearSaveCurrentClipState();
  }, [handler]); // <-- the bug: reactive to an identity that churns every render
}

/**
 * Whole-store subscriber (reproduces FocusScreen.jsx:59-66) that registers a handler whose
...
function ChurningSubscriber({ registerImpl, counter }) {
  useFocusStore(); // selector-less subscription: any store write re-renders this component
  counter.n += 1;
...
describe('T6190 active-save-handler registration', () => {
  ...
  it('GREEN: useRegisterActiveSaveHandler registers stably and does NOT loop', () => {
    ...
    expect(typeof useFocusStore.getState().activeSaveCurrentClipState).toBe('function');
    expect(useFocusStore.getState().activeSaveCurrentClipState()).toBe('saved');
  });
  ...
  it('GREEN: the stable registration always calls the LATEST handler (ref semantics)', () => {
    ...
    const registered = useFocusStore.getState().activeSaveCurrentClipState;
  ...
  it('GREEN: clears the registration on unmount', () => {
    ...
    expect(useFocusStore.getState().activeSaveCurrentClipState).toBeTypeOf('function');
    act(() => unmount());
    expect(useFocusStore.getState().activeSaveCurrentClipState).toBeNull();
  });
});
