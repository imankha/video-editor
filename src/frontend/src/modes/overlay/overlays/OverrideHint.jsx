 ...
 * manually overridden. Rendered in the dimmed area OUTSIDE the spotlight circle
 * (bottom-center), low z (below the edit handles/grip), and NON-interactive
 * (`pointer-events-none`) so it never steals a tap from the circle, a player box, or
 * video tap-nav.
 *
 ...
 * show — that logic (tracking ON + a region exists + not yet overridden) lives in the
 * parent so the "seen" flag stays view-state.
 */
export default function OverrideHint({ visible, text }) {
  ...
  return (
    <div
      ...
      aria-hidden="true"
      className={`pointer-events-none absolute bottom-3 left-1/2 -translate-x-1/2 z-[5] max-w-[90%] px-3 py-1.5 rounded-full text-xs text-center text-white/70 bg-black/40 backdrop-blur transition-opacity duration-300 ${
