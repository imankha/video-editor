...
// T7820 review: cap the captured frame near tile size. Phone game footage is
// routinely 4K; an uncapped capture stores a ~MB-scale data URL in the store,
// decodes a ~33MB bitmap for a ~300px tile, and toDataURL on a 4K canvas is a
// synchronous main-thread encode fired right as hashing/upload starts.
...
export async function captureVideoFrame(file) {
  ...
  try {
    ...
    // 'metadata' is sufficient — the explicit seek pulls the needed range anyway
    // (same choice as videoUtils.js / videoMetadata.js / PosterFramePreview).
