...
// match dates printed in their own titles.
function game(id, gameDate, createdAt = '2026-06-11T18:00:00Z') {
...
describe('groupGamesByMonth — match date is the organizing key (T7290)', () => {
  ...
  it('places a game with no match date by its upload date, without dropping it', () => {
    ...
    const games = [
      game(1, '2026-05-09'),
      game(2, null, '2026-02-14T09:00:00Z'),
      game(3, '2026-03-21'),
    ];
  ...
  it('treats an empty-string match date the same as a missing one', () => {
    const { groups, order } = groupGamesByMonth([game(9, '', '2026-01-05T09:00:00Z')]);
  ...
  it('breaks a same-match-day tie on upload time, newest first, stably', () => {
    ...
    const games = [
      game(1, '2026-04-18', '2026-06-11T10:00:00Z'),
      game(2, '2026-04-18', '2026-06-11T18:00:00Z'),
      game(3, '2026-04-18', '2026-06-11T14:00:00Z'),
    ];
