...
**Branch CI selects applicable jobs using `scripts/ci_policy.py`.** It captures the base/head once and records a routing manifest. Instructions and orchestration tooling have their own checks; frontend/backend changes run their complete affected-layer suites. Unknown shared executable paths conservatively run both layers. `ci-ready` requires every applicable job to succeed; missing, failed, cancelled, or unexpectedly skipped checks are not green. A passing instruction audit is structural evidence, not proof of agent quality. Do not select individual application tests for CI by filenames; local runs remain curated. Master CI retains its full post-merge sweep. Fix loops rerun failing tests and affected consumers. Details: `.claude/skills/run-tests/SKILL.md`.
