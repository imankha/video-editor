...
- **Mobile**: project has a responsiveness skill (`.claude/skills/responsiveness`) — mobile-first patterns and a 360-428px testing workflow. The epic requires mobile parity everywhere and mobile-PRIMARY design on public share pages (EPIC decision #14). Touch drag-and-drop is unreliable; always pair drag with button fallbacks.
