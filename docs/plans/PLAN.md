...
| T6820 | ↳ [Hover preview for Not Started drafts (source clip window)](tasks/draft-stage-display/T6820-hover-preview-not-started-source-clip.md) | 5 | 4 | 1.25 | TODO | [ ] | Extend the T6420 preview chain (final → working → NEW: source clip window) using the existing bounded clip-window proxy (`clips.py::stream_working_clip_bounded`); needs `ClipSummary` to expose first-clip source offsets + `TilePreviewVideo` start/end window props (seek + loop). NOT freeze material. Coordinate with the Tile Video Preview epic. |
...
| T4946 | ↳ [Access control + credits](tasks/collection-download/T4946-access-control-and-credits.md) | 6 | 3 | 1.2 | TODO | [ ] | **Blocked on an unresolved credit-model question - do not start.** User: "Anyone with permissions to the clip and who is signed in and has the credits can download" - widens owner-only to a permission check, and implies a credit charge that contradicts the original "free" recommendation the user separately seemed to accept. Needs a human decision before implementation (see EPIC.md Decision 4). |
| T4947 | ↳ [Cache stitched downloads](tasks/collection-download/T4947-cache-stitched-downloads.md) | 5 | 3 | 1.0 | TODO | [ ] | Disposable R2 cache keyed by member-set hash (no DB row, no migration); skip the credit charge on a cache hit once T4946 settles the credit model. Depends on T4946. |
...
| T7010 | [Clip-save "couldn't save" error + both clips landed under the same game (needs better logging)](tasks/T7010-clip-save-conflict-and-game-misattribution.md) | 6 | 4 | 1.5 | TODO | [ ] | Filed 2026-08-14 from user report while live-testing T6820. Backend log confirms the "couldn't save" error was a real CAS `SYNC_CONFLICT`/`stale_baseline` (R2 11 versions ahead of the container's baseline — matches the known multi-container-QA-one-account collision pattern, several `/dotask` stacks were live simultaneously) that auto-healed via a full DB re-download + retry (200 on retry, no data lost). Separately, both clips ended up grouped under the same game ("Legends Mar 28") though the user believed they created them in 2 different games — 2 unconfirmed hypotheses (pre-existing shared game_id vs. a stale frontend game-context after the error/retry/heal cycle, a T4060-class bug). User explicitly asked for better logging: frontend-believed game id alongside backend-read game id at save time, and a CRITICAL log line when a mid-request DB heal discards in-flight work. |
