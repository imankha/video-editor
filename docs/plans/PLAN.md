...
| T800 | [Remove Legacy Extraction Infrastructure](tasks/T800-remove-extraction-infrastructure.md) | TESTING | 5 | 5 | Dead extraction code across ~15 files: response models, retry endpoint, WebSocket manager, modal_queue functions, tests, stale comments. T790 removed triggers; this removes everything else. |
| T1000 | [DRY Quest Definitions](tasks/T1000-dry-quest-definitions.md) | TODO | 6 | 4 | Quest defs duplicated in 3 files (frontend, quests.py, admin.py); admin copy is stale/wrong. Single source of truth in backend, derive everything else. |
