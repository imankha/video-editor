...
| T7050 | [Collection download has no in-app progress feedback](tasks/T7050-collection-download-progress-bar.md) | 4 | 3 | 1.3 | WIP | [ ] | Filed 2026-08-15 at user request — collection download (T4945) gives zero in-app feedback while in flight, just a bare browser download once it completes. Add a progress bar (determinate if `Content-Length` is available, indeterminate otherwise); check for an existing progress pattern on the export flow before building new UI. Pure polish, no data risk — lands after T7040 (fix the "Failed to fetch" bug before polishing the loading state). |
