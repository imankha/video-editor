...
| T1960 | [Migrate Global SQLite to Fly Postgres](tasks/for-launch/T1960-migrate-auth-to-fly-postgres.md) | 8 | 6 | 1.3 | TESTING | [ ] | Move auth.sqlite + sharing.sqlite to Fly Postgres (~$2-4/mo). Eliminates restart fragility, concurrent write contention, and O(users) R2 syncs. Per-user SQLite+R2 stays (correct with session pinning). Gates alpha exit. |
