...
| T6420 | [Play video preview on hover / long-press for My Drafts and My Reels tiles](tasks/T6420-tile-video-preview-hover-longpress.md) | 6 | 4 | 1.5 | TODO | [ ] | User-ordered 2026-08-03, placed AFTER the reshoot by design (transient hover interaction; the tutorials don't demo it — the milestone's touch-up rule covers it if a quest ends up showing it). Inline muted looping preview inside the tile: fine pointer = hover with ~400ms intent delay, coarse pointer = long-press (rides the SAME gesture that reveals actions — T5910/T6300 — recommended: one gesture does both; confirm T6300's shipped treatment first). Shared `TilePreviewVideo` primitive for both tiles (T6320 precedent); source is the existing `/api/downloads/{id}/stream` both players already use; DraftTile gated on `final_video_id`. Hard rules baked into the task file: poster-first reveal (no black flash), teardown must RELEASE the stream, one active preview max, `preload="none"` at rest (T6290), no watched-marking or any write (ephemeral only), absolute-not-portaled (T5900 containing-block trap), `prefers-reduced-motion` opt-out, real-browser evidence required (T5380). |
