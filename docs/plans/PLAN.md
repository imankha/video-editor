...
| T7020 | [Remux uploaded game videos with faststart (moov at front)](tasks/T7020-game-video-faststart-remux.md) | 6 | 4 | 1.5 | TODO | [ ] | **Sequenced first after T5140 (user-ordered 2026-08-15)** — deliberately held out of the next deploy for more thorough testing, not blocked on anything technical. A synchronous version exists and works (branch `feature/T7020-game-video-faststart-remux`, commit `7f2aeb4e`, kept on origin, not merged) — CI green, reliability-hardened (fixed a real retry-classifier gap found live-testing it, see commit message), but adds ~as much wall-clock to `finalize_upload` as the original upload itself (measured 65-78s remux on a 278MB test, matching a 58.5s upload) because the remux round-trips the file through R2 twice more. User reviewed a design note comparing that against dispatching the remux to Modal (mirrors T4945's `stitch_members` pattern, keeps the response near-instant, never adds upload-time cost) and prefers the Modal-dispatch redesign — full comparison + sequence diagrams: [design artifact](https://claude.ai/code/artifact/27a9f3e5-38fb-44bd-8dcb-50655873f81c). Redesign work + thorough testing to happen when this task is picked back up. |
...
| T4946 | ↳ [Access control (permission + sign-in, free)](tasks/collection-download/T4946-access-control-and-credits.md) | 6 | 2 | 3.0 | STAGING | [ ] | Merged 2026-08-15 (`3589ade6`). Code-expert audit confirmed permission already collapses to ownership STRUCTURALLY (`download_collection` always reads the caller's own profile DB; `final_videos` never holds a foreign reel) and sign-in is already enforced by `db_sync` middleware (401, path not allowlisted) — no unguarded hole existed. Deliverable: 5 regression tests locking the invariants + resolved docstring TODO. No production logic changed. |
| T4947 | ↳ [Cache stitched downloads](tasks/collection-download/T4947-cache-stitched-downloads.md) | 5 | 2 | 2.5 | WIP | [ ] | Disposable R2 cache keyed by member-set hash (no DB row, no migration). Credit-recharge-exemption half of the original scope is moot (downloads are free, Decision 4). Depends on T4946. |
...
**Queued directly after the reshoot:** [T7020](tasks/T7020-game-video-faststart-remux.md) (user-ordered 2026-08-15) is FIRST — see its row above for why it's deliberately held rather than shipped as-is. Then the [JIT Migration epic](tasks/jit-migration/EPIC.md) (T5081-T5089, user-ordered 2026-08-04) — not a UI-runway member, placed immediately after T5140 and ahead of Tile Video Preview. [Collection Download](tasks/collection-download/EPIC.md) (above) is a further member of this same "after the reshoot" group — relative order between it and JIT Migration not yet specified by the user.
...
| T7000 | [`_js_path()` loud-failure guard no longer raises at Fly deploy depth](tasks/T7000-js-path-guard-broken-fly-depth.md) | 4 | 2 | 2.0 | STAGING | [ ] | Merged 2026-08-15 (`e7e957b5`). Root cause: the test's mock used a Windows-style path (`C:\app\...`), which resolves differently than a real POSIX Fly path under Linux CI's `.resolve()` — the depth check never fired, so the guard never raised. The guard itself was correct all along. Fixed the mock to be POSIX-absolute; `known-failures.md` row removed (5 independent confirmations resolved: master, T6990, T7040, T4946, T7020). |
| T7040 | [Collection download fails with "TypeError: Failed to fetch"](tasks/T7040-collection-download-failed-to-fetch.md) | 8 | 5 | 1.6 | STAGING | [ ] | Merged 2026-08-15 (7718412b). Confirmed root cause (Hypothesis D): `cleanup_stale_exports` made a blocking Modal SDK call per stale export job inside an `async def` route, unawaited - froze uvicorn's single-worker event loop and starved a concurrent collection-download request. Fixed via `anyio.to_thread.run_sync`; also moved `collections.py`'s stitch/compose work out of the `StreamingResponse` generator so a failure there now raises a clean HTTP 500 instead of a bare client-side "Failed to fetch". CI green apart from the pre-existing T7000 known-failure. |
| T7050 | [Collection download has no in-app progress feedback](tasks/T7050-collection-download-progress-bar.md) | 4 | 3 | 1.3 | STAGING | [ ] | Merged 2026-08-15 (`4494a74b`). Backend sends no `Content-Length`, and T7040 builds the whole file server-side before the first byte, so the no-total case shows a spinner (not a bar — user feedback found a static pulsing bar for the ~1-minute pre-first-byte wait read as stuck, not busy) with a live byte readout once bytes arrive. Progress is per-card state, matching the existing per-card busy-flag pattern. |
| T7030 | [Intro card image doesn't load; scrub-back leaves video blank](tasks/T7030-intro-card-image-scrub-back-blank.md) | 7 | 5 | 1.4 | STAGING | [ ] | Merged 2026-08-15 (832775a4). Root cause: `MotionPreview`'s visible `<img>` gated its reveal solely on `onLoad`, but T6960's preload gate always makes it a cache hit — a cache-complete `<img>` may never fire `load` for a just-attached React listener (WebKit/Safari/mobile). Fix reveals off `img.complete` too. The `/downloads/{id}/stream` 404 (Hypothesis A) traced to a data condition (missing `final_videos` row) on that one staging download, not a frontend bug — out of scope, separate data-integrity look if it recurs. |
