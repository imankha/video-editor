...
| T5215 | ↳ [Attachment + resolution](tasks/player-intro/T5215-intro-attachment.md) | 7 | 5 | 1.4 | TODO | [ ] | Wave 4. Wire final_videos.intro_card_id (column already shipped in v034) + collection-definition field + resolution helper + reel/collection pickers. NULL = inherit the profile default, 0 = explicitly no intro - never collapse the two. Depends on T5195. |
| T5220 | ↳ [Apply the intro at every egress](tasks/player-intro/T5220-add-intro-integration.md) | 7 | 5 | 1.4 | TODO | [ ] | Wave 5. Serve-time prepend, playback pre-roll in CollectionPlayer, T4945 stitch seam, public-exposure notice. Depends on T5210 + T5215. |
...
| T5230 | ↳ [Children's-data compliance hardening](tasks/player-intro/T5230-childrens-data-compliance.md) | 7 | 4 | 1.8 | TODO | [ ] | Wave 6, gates public launch. Guardrails + the recorded COPPA analysis: grad-year default (DOB opt-in + app-encrypted), consent gate, retention/deletion wired into privacy.py export+purge, no face-recognition guardrail, privacy-policy update (human sign-off). Threads through T5190 + T5220. |
...
| T6530 | [UX pass: how should the intro card feature actually be surfaced?](tasks/T6530-intro-card-discoverability-ux.md) | 7 | 4 | 1.8 | TODO | [ ] | **User request 2026-08-05. DO AFTER T5215+T5220** - evaluate with the whole flow working, not before. The epic shipped its machinery before anything decided how a user FINDS it; each task placed its surface reasonably and locally and the sum is not a considered flow. Symptom: the user could not find the feature at all on a build that definitely contained it - entry is ManageProfilesModal -> open a profile -> a button at the bottom, rendered ONLY when that profile is active, degrading to a line of grey text otherwise (ManageProfilesModal.jsx:433). Decide where card MANAGEMENT vs card SELECTION each belong, first-run for a user with zero cards, the non-active-profile case, and naming. ui-designer required; proposal with visuals approved by the user BEFORE any code. |
