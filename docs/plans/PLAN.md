...
| T3780 | [Framing/Overlay Clarity P1: Reduce On-Screen Noise](tasks/T3780-framing-overlay-clarity-p1.md) | 6 | 4 | 1.5 | TODO | [ ] | T3700 P1 (split out): hide confidence % (and any pixel readouts) behind Advanced; make quest hints replayable; deep-link "Open your reel"/"Library" nav. Cleanups: fix stale `open_overlay` quest copy (still says "click the card under Drafts" though framing now auto-advances) + CropLayer "Set Crop Keyframes" jargon placeholder. |
...
|  | **[Quests Latency](tasks/quests-latency/EPIC.md)** |  |  |  |  |  | Quests endpoints are the slowest non-video calls in the framing/overlay loop (HAR 2026-06-17). One branch `feature/perf-quests-latency`: make each call cheaper (T1536) AND fire fewer of them (T1537). Sequenced T1536 → T1537; combined before/after measured once. Quests half of the perf batch ([coordination](tasks/perf-batch-har-2026-06-17.md)). |
| T1536 | ↳ [Quests /progress + /achievements Latency](tasks/quests-latency/T1536-quests-progress-endpoint-latency.md) | 6 | 3 | 2.0 | TODO | [ ] | Prod HAR: `/quests/progress` up to 699ms, `/achievements` up to 636ms (all server `wait`), ~400ms above the ~200ms baseline, fired on most gestures. Root cause: `/progress` opens user.sqlite TWICE + profile.sqlite once per call (R2-restore overhead), NOT `_check_all_steps` (verified: no dead quests/steps post-T3700, 4 cheap queries). Fix: profile-first (attribution), then merge the two user.sqlite opens into one (helper `get_completed_and_claimed_quest_ids`); conditionally skip profile.sqlite for fully-completed users. Merit gate: deterministic connection-count test (2→1). No persistence-model change. |
| T1537 | ↳ [Consolidate Achievement POSTs](tasks/quests-latency/T1537-consolidate-achievement-posts.md) | 5 | 3 | 1.7 | TODO | [ ] | Each crop/speed/color/shape gesture fires 2 POSTs (`/actions` + `/quests/achievements/*`) plus a preflight each. Derive the achievement from the action type server-side and fold the `INSERT OR IGNORE` into the action handler (framing_action + overlay_action reuse their existing profile.sqlite conn; non-blocking). Replace the 4 frontend `recordAchievement` calls with a dedup-gated progress-refresh helper (Finding B: the old call also triggered fetchProgress); keep lifecycle ones. Merit gate: frontend request-count test (2→1). Pairs with T1536. |
...
| ID | Task | Impact | Cmplx | Pri | Status | Migr | Description |
|------|------|------|------|------|------|------|------|
| T3760 | [Framing Clip Cold-Load Over-Fetch](tasks/T3760-framing-clip-cold-load-overfetch.md) | 9 | 5 | 1.8 | TODO | [ ] | Framing plays the clip via the direct R2 presigned URL (T3250, to escape the ~590 KB/s Fly proxy); for a clip ~66% into a 3 GB source the browser issues an open-ended range at byte ~2.0 GB → R2 serves to EOF → over-buffers. **Prod HAR (app.reelballers.com 2026-06-17) confirms it recurs on every seek**: 3 backward open-ended ranges, each ~1.03 GB to EOF, first streamed 55.8s — user reports felt stalls (Impact 8→9). Bounded `/stream` proxy (T1430) exists but is only the error fallback. Spike→decide: faststart (T2580) + edge range-clamp via CDN Worker (re-opens T2560's "likely skip" — skipped on egress cost, but this is latency). Fix must clamp seek ranges too. Do NOT revert to the Fly proxy. |
...
| T1515 | [Suppress Analytics During Impersonation](tasks/T1515-suppress-analytics-during-impersonation.md) | 6 | 3 | 2.0 | TODO | [ ] | Follow-up to T1510: admin impersonation actions must be invisible to analytics. Guard record_milestone() (backend ContextVar from session.impersonator_user_id) + track() (frontend authStore.impersonator). Protects activity data that lifecycle/funnel/viewer analytics rely on. Audit log still records. |
...
| ID | Task | Impact | Cmplx | Pri | Status | Migr | Description |
|------|------|------|------|------|------|------|------|
|  | **[R2 CDN Video Serving](tasks/r2-cdn/EPIC.md)** | 9 | 5 | 1.8 |  |  | Custom domain + HMAC auth + HTTP/2 + CDN caching. Presigned URL streaming done in T3250; this epic adds edge infrastructure. Depends on T3250. |
| T2550 | ↳ [CDN + Auth Worker](tasks/r2-cdn/T2550-r2-custom-domain-cdn.md) | 8 | 4 | 2.0 | TODO | [ ] | Custom domain (`cdn.reelballers.com`) + HMAC auth Worker + HTTP/2 + CDN caching. Auth-only Worker (no byte proxying). |
| T2560 | ↳ [Edge Byte-Range Clamping](tasks/r2-cdn/T2560-edge-video-worker.md) | 7 | 5 | 1.4 | TODO | [ ] | Port 3-window clamping to Worker. **Likely skipped** -- T3250 drops clamping (user-owned content, zero egress). |
...
- `T1536, T1537, T3760, T3770` - **Perf batch (HAR 2026-06-17)** — coordinated rollout across 2 branches / 3 conversations. See [perf-batch-har-2026-06-17.md](tasks/perf-batch-har-2026-06-17.md). Branch `feature/perf-quests-latency`: T1536 → T1537 (one conversation, quests endpoint latency + achievement POST consolidation). Branch `feature/perf-page-load`: T3760 (framing clip cold-load over-fetch, re-opens T2560 clamp on latency grounds) + T3770 (StrictMode duplicate page-load fetch confirm, disjoint files).
