...
| T1490 | [First clip-stream request returns 401, frontend hangs](tasks/T1490-video-stream-first-request-401.md) | IN_PROGRESS | 5.0 | Every new clip load: first GET /stream 401 (no session cookie), retry 206. Browser auto-retries so user only sees slow spin. Fix A: set crossOrigin=use-credentials on same-origin proxy URLs in detached probe; bundle 401 visibility logging |
| T1500 | [Persist clip dimensions, eliminate metadata probe](tasks/video-load-reliability/T1500-persist-clip-dimensions.md) | TODO | 2.5 | Follow-up to T1490: persist width/height/fps on working_clips, backfill existing rows, skip frontend metadata probe when fields present. Removes N media probes per project load |
