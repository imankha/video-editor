...
| T10940 | [Silent app update did not fire; user had to hard-refresh](tasks/T10940-silent-update-did-not-fire-hard-refresh-needed.md) | 8 | 4 | 2.0 | TODO | [ ] | User report 2026-09-21 during a burst of 5 master pushes: no popup (by design since T8460) AND no silent reload (T9360's 30 s visible poll + quiescence gate should have done it). Prime suspect: frontend/backend deploy halves finish at different times, the SW probe answers "no bundle yet" and `PROBE_MIN_GAP_MS` locks re-probing for 5 min (Gap C only shortens a still-installing miss). Needs env + timing from the user or a staged repro; the sw-gate e2e with builds published in the wrong order is the automated form. |
