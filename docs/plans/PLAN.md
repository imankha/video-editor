...
| T4946 | ↳ [Access control (permission + sign-in, free)](tasks/collection-download/T4946-access-control-and-credits.md) | 6 | 2 | 3.0 | WAITING ON USER | [ ] | Pushed 2026-08-15 (`052e5124`). Code-expert audit confirmed permission already collapses to ownership STRUCTURALLY (`download_collection` always reads the caller's own profile DB; `final_videos` never holds a foreign reel) and sign-in is already enforced by `db_sync` middleware (401, path not allowlisted) — no unguarded hole existed. Deliverable: 5 regression tests locking the invariants + resolved docstring TODO. No production logic changed. CI green apart from the pre-existing T7000 known-failure. |
...
| T7000 | [`_js_path()` loud-failure guard no longer raises at Fly deploy depth](tasks/T7000-js-path-guard-broken-fly-depth.md) | 4 | 2 | 2.0 | WAITING ON USER | [ ] | Pushed 2026-08-15 (`836b58e4`). Root cause: the test's mock used a Windows-style path (`C:\app\...`), which resolves differently than a real POSIX Fly path under Linux CI's `.resolve()` — the depth check never fired, so the guard never raised. The guard itself was correct all along. Fixed the mock to be POSIX-absolute. CI green. Merging this one unblocks the known-failure that's been hitting every other branch's CI (4 independent confirmations so far). |
...
| T7050 | [Collection download has no in-app progress feedback](tasks/T7050-collection-download-progress-bar.md) | 4 | 3 | 1.3 | WAITING ON USER | [ ] | Pushed 2026-08-15 (`6198eb19`). Backend sends no `Content-Length` on the collection-download stream, so the bar is indeterminate (not a dishonest fake percentage). Reused the existing `GlobalExportIndicator` progress-bar treatment; progress is per-card state (not hook-global), matching the existing per-card busy-flag pattern. 15 component/hook tests cover all 3 UI states — worker could not live-drive end-to-end (no backend stitch stack in its container); verify live on staging. CI green apart from the pre-existing T7000 known-failure. |
...
| T7020 | [Remux uploaded game videos with faststart (moov at front)](tasks/T7020-game-video-faststart-remux.md) | 6 | 4 | 1.5 | WAITING ON USER | [ ] | Pushed 2026-08-15 (`9cbf0001`). New `game_remux.py` service, wired into `finalize_upload` outside the DB transaction, fails open on any error (download/ffmpeg/upload failure all keep the original file, logged loudly). Measured: `-c copy +faststart` runs at ~1440MB/s — inline cost is dominated by the R2 transfer, not ffmpeg itself. 10 new tests with real ffmpeg + 83 curated regression tests green. CI green apart from the pre-existing T7000 known-failure. |
