...
**UI Runway (user-ordered 2026-08-03):** every near/mid-term task that visibly changes the UI lands FIRST, then the tutorials are reshot once against settled screens — see the [UI Runway -> Tutorial Reshoot](#ui-runway---tutorial-reshoot-user-ordered-2026-08-03) milestone below.
...
## UI Runway -> Tutorial Reshoot (user-ordered 2026-08-03)
...
|------|------|------|------|------|------|------|------|
|  | **[Player Intro + Rich Text](tasks/player-intro/EPIC.md)** | 7 | 6 | 1.2 |  |  | **REDESIGNED 2026-08-03** (user direction; design review + mockups: https://claude.ai/code/artifact/93478a34-c7e5-406f-a56b-3c3724e4b6dd). Was: ONE intro auto-composed from profile athlete fields, burned into a multiclip export. Now: a LIBRARY of N intro cards (image + templated slots of styled text), a different card attachable to every reel and collection with one profile default, and the rich-text system built ONCE and reused as an Overlay timeline layer. Approved decisions: (1) intro applies at DOWNLOAD/PLAYBACK mirroring append_branded_outro - never burned at export, so swapping a card is free and every existing reel gets one with no backfill; (2) card layout = 3 templates with named slots, no free dragging in v1; (3) card-first - the 10-column athlete field set is NOT built (ships sooner, one less migration, less minor PII); (4) Overlay text range = free drag that SNAPS to clip boundaries; (5) backend text = Pillow raster to RGBA PNG (NOT ffmpeg drawtext, NOT headless browser) - one PNG feeds ffmpeg for the card and OpenCV per-frame for Overlay; (6) 6 curated licence-checked TTFs shipped in-repo, same files served to the browser; (7) cards are per-profile (profile_db v034); (8) intro_card_id NULL=inherit default, 0=explicitly none. Key find: working_videos.text_overlays already exists as an unwritten placeholder, so the Overlay layer needs NO migration. 5 children -> 9. COPPA posture unchanged (likely N/A but children's-data posture adopted; risk is PUBLIC EXPOSURE of a minor's photo + free text, not storage). |
| T5180 | ↳ [Rich text engine (TextSpec + fonts + one renderer/preview)](tasks/player-intro/T5180-rich-text-engine.md) | 7 | 5 | 1.4 | TODO | [ ] | NEW 2026-08-03. Foundation, no user-visible feature: normalised TextSpec model (replaces the pixel-based TextOverlay stub in schemas.py), 6-face licence-checked font catalogue + one fonts.json manifest read by both sides, app/services/text_render.py (Pillow -> RGBA PNG, wrap/stroke/shadow, content-hash cached), RichText.jsx preview loading the SAME TTFs via @font-face, and a Playwright PARITY TEST asserting backend raster and browser box agree per font. Consumed by T5210 (card) + T5225 (overlay text) - if those ever need different text code paths the design failed. No migration, no new dependency (pillow/numpy already present). Runs parallel with T5190. Architect gate on the TextSpec shape. |
| T5190 | ↳ [Card image upload + consent](tasks/player-intro/T5190-card-image-upload-consent.md) | 7 | 3 | 2.3 | TODO | [ ] | **RESCOPED 2026-08-03** (was "Athlete profile fields + photo + consent"). Epic decision 3 dropped the 10 athlete columns and the user_db migration; what remains is what every card needs - a small image-upload endpoint (decode-verify, re-encode ~1440px, per-profile {APP_ENV}/users/{uid}/profiles/{pid}/intro/ prefix - mandatory or it 404s cross-profile; copy the poster-upload pattern, NOT the blake3 uploadManager), a delete path for T5230's purge, and the parental-consent attestation gating all intro use. M-tier, likely no migration. Runs parallel with T5180. |
| T5195 | ↳ [Intro card library: schema, CRUD, default](tasks/player-intro/T5195-intro-card-library.md) | 7 | 4 | 1.8 | TODO | [ ] | NEW 2026-08-03. profile_db **v034**: intro_cards table (name/template/image_key/image_cutout_key/text_elements msgpack/duration/is_default) + final_videos.intro_card_id in the same migration, plus matching DDL in ensure_database(). REST CRUD with SURGICAL updates, atomic single-default enforcement, delete clears referencing intro_card_ids and the R2 image. NULL vs 0 on intro_card_id carry different meanings (inherit vs explicitly-none) - collapsing them makes opting one reel out impossible. Column-guard every hot read for the deploy->migrate window; runner hands up(conn) a TUPLE row factory. Migration agent + Architect. Depends on T5180 + T5190. |
| T5205 | ↳ [Intro card editor UI](tasks/player-intro/T5205-card-editor-ui.md) | 7 | 5 | 1.4 | TODO | [ ] | NEW 2026-08-03. Library grid (rendered browser previews, default marked, new/edit/duplicate/set-default/delete - duplicate covers the retyping cost of decision 3) + the editor: template picker (3 layouts, switching preserves typed text), stage composing background+image+one RichText per slot, right rail editing the selected slot's TextSpec (text, font shown in its own face, size S/M/L/XL, colour swatches+picker, align), image picker + consent gate, and a BROWSER motion preview sharing T5210's timing constants. Gesture-based surgical PATCH per edit, no reactive persistence. ui-designer recommended; real-browser verification mandatory (T5380). Frontend-only. Depends on T5195. |
| T5210 | ↳ [Intro card render engine](tasks/player-intro/T5210-intro-card-generation.md) | 7 | 6 | 1.2 | TODO | [ ] | **REWORKED 2026-08-03**: renders any card DOCUMENT (image + N TextSpec in template slots), not profile fields, with text as pre-rendered RGBA PNG layers from T5180 - ffmpeg composites and animates, never draws a glyph. app/services/player_intro.py mirrors branded_outro.py: probe-match, PREPEND [card][main] via _concat_copy/_concat_reencode/_validate_concat, cache per (card-content-hash x w x h x fps x pix_fmt x audio) + _CARD_VERSION, non-fatal always. Motion IS the deliverable (photo push-in/Ken Burns + staggered text + white flash), sharing T5240's vocabulary; timing constants shared with T5205's preview, never duplicated. T5240 ffmpeg landmines recorded in the task file. Backend-only, no schema change. Depends on T5195. |
| T5215 | ↳ [Intro attachment: reel, collection, default](tasks/player-intro/T5215-intro-attachment.md) | 7 | 4 | 1.8 | TODO | [ ] | NEW 2026-08-03. ONE resolve_intro_card helper (0 -> none, NULL -> profile default, id -> that card; dangling id warns and degrades, never self-repairs) used by every consumer. Reel: surgical PATCH /api/downloads/{id}/intro; the attachment must SURVIVE RE-EXPORT (final_videos inserts a new version row - the finalize path must carry intro_card_id forward; top regression risk). Collection: intro_card_id on CollectionDefinition riding the existing collection_definition JSONB (no Postgres migration), FROZEN at share creation. Pickers in My Reels + the collection share dialog, public-exposure notice, consent gate. Depends on T5210. |
| T5220 | ↳ [Apply the intro at every egress](tasks/player-intro/T5220-add-intro-integration.md) | 7 | 5 | 1.4 | TODO | [ ] | **REWORKED 2026-08-03** from burn-at-export to apply-at-serve (epic decision 1). Four doors, all resolving via T5215: owner download (prepend in the SAME concat pass as append_branded_outro -> [intro][reel][outro], non-fatal, HTTP 200 always), playback pre-roll component mirroring BrandedEndCard in reverse (share page, CollectionPlayer, edge function; owner surfaces excluded), share-page download (coordinate with T6360 which already routes it through the backend), and the T4945 stitched-collection seam (intro = first segment, once per collection). Factor ONE serve-time helper so intro+outro+T6360 metadata compose in a single pass. Public-exposure notice wherever an intro is attached or shared. Manual verification on a real link + real download mandatory. Depends on T5215. |
| T5225 | ↳ [Overlay text layer (rich text over a clip range)](tasks/player-intro/T5225-overlay-text-layer.md) | 7 | 6 | 1.2 | TODO | [ ] | NEW 2026-08-03 - user requirement 5. Third layer in OverlayTimeline (the component already reserves the slot at :103/:153), free-drag range that SNAPS to clip boundaries mirroring RegionLayer's pointer pattern, selection opening the SAME right rail as the card editor (same TextSpec, same component - not a copy). Persists into the EXISTING working_videos.text_overlays BLOB (already returned by /overlay-data, already counted in projects.py:332, nothing writes it) -> **NO MIGRATION**. Burn-in in BOTH per-frame OpenCV loops (Modal _process_overlay + local _overlay_sync) by alpha-blending T5180's cached layer - rasterise once per spec, never per frame; needs a Modal redeploy (ask user). Unlike the intro, overlay text is burned, so editing after export means re-export - say so AT EDIT TIME. Depends on T5180 ONLY, so it can run early. |
| T5230 | ↳ [Children's-data compliance hardening](tasks/player-intro/T5230-childrens-data-compliance.md) | 7 | 4 | 1.8 | TODO | [ ] | Gates public launch. **Retargeted 2026-08-03**: decision 3 removed the structured athlete field set, so there is no DOB/height/high-school in the DB and the DOB-encryption clause is satisfied by not collecting it. Full weight now on consent record, public-exposure warning, no-face-recognition guardrail, privacy-policy update (human sign-off), and retention/deletion in privacy.py export+purge covering intro_cards rows, their free-text text_elements, AND the R2 images under the per-profile intro/ prefix. R2 SSE + TLS is the encryption baseline. M/L-tier. Threads through T5190/T5215/T5220. |
| T5200 | ↳ [Player cut-out ("player outline")](tasks/player-intro/T5200-player-cutout.md) | 6 | 5 | 1.2 | TODO | [ ] | **Deprioritised 2026-08-03 (epic decision 11): blocks nothing** - the card engine works with a plain photo, so this ships whenever it comes up. Optional background removal so the uploaded photo becomes a clean cut-out like the reference; server-side segmentation/matting (rembg/u2net-class) with upload-your-own-PNG fallback. Matting is NOT face-recognition (stays clear of biometric laws). Result is now stored as intro_cards.image_cutout_key, NOT on the profile. Greenfield image processing. M/L-tier. |
...
|  | **[Tile Video Preview](tasks/tile-video-preview/EPIC.md)** | 6 | 6 | 1.0 |  |  | EPIC, user-ordered 2026-08-03, deliberately AFTER the reshoot (an undemoed preview contradicts nothing in the tutorials — users discover it naturally; touch-up rule covers it if ever needed). Tiles play their video inline — muted, looping, in place — on glance. Design authority = YouTube/Netflix prior art (decisions table in EPIC.md): in-place not Netflix-expansion, muted-always (Netflix's sound-on lesson), warm-early/reveal-late (fetch at ~100ms hover, reveal at ~450ms — user requirement: no visible wait), touch = in-viewport autoplay NOT long-press (neither product uses long-press; T6300's discoverability lesson; leaves the action-reveal gesture untouched). Shared invariants bind all children: one active preview, teardown releases the stream, zero requests at rest (T6290), ephemeral-only (no watched-marking), never portaled (T5900), reduced-motion opt-out, real-browser evidence (T5380). |
| T6420 | ↳ [TilePreviewVideo primitive + desktop hover](tasks/tile-video-preview/T6420-preview-primitive-desktop-hover.md) | 6 | 4 | 1.5 | TODO | [ ] | Child 1/3. Shared `TilePreviewVideo` + `useTilePreview` (T6320 precedent — sibling tiles must not diverge) wired to fine-pointer hover on DraftTile (gated on `final_video_id`) + ReelTile; source is the existing `/api/downloads/{id}/stream`. Two-stage activation: grace 0-100ms (mouse crossings fire nothing) -> warm (src + preload, poster untouched) -> reveal at ~450ms with first-frame crossfade (poster-first, no black flash/spinner; R2 ~266ms TTFB makes reveal typically instant). Ships the single-active registry T6430 reuses. Touch byte-identical in this child. M-tier, frontend-only. |
| T6430 | ↳ [Touch: in-viewport autoplay of most-visible tile](tasks/tile-video-preview/T6430-touch-in-viewport-autoplay.md) | 5 | 4 | 1.3 | TODO | [ ] | Child 2/3, depends on T6420. Coarse pointer: IntersectionObserver coordinator crowns the single most-visible eligible tile after scroll settles (~300-500ms); flick-through must not strobe or fire a stream per tile. Handles vertical grid + horizontal My Reels carousels (nested). Coordinator is ephemeral (context/module, NOT a store). Long-press action reveal (T5910/T6300) stays byte-identical. Real-device pass required (iOS momentum autoplay). |
| T6440 | ↳ [Autoplay-previews setting + data-saver](tasks/tile-video-preview/T6440-autoplay-setting-data-saver.md) | 3 | 2 | 1.5 | TODO | [ ] | Child 3/3. The Netflix lesson: "Autoplay previews" toggle (default ON, real preference, existing settings path, gesture-based write) + `navigator.connection.saveData` hard-off (Chromium-only; absence = no signal). Both gates centralized in the T6420 activation check alongside reduced-motion. S/M-tier. |
...
| ID | Task | Impact | Cmplx | Pri | Status | Migr | Description |
|------|------|------|------|------|------|------|------|
| T6450 | [build-lockstep guard checks an artifact production builds never regenerate](tasks/T6450-build-lockstep-guard-reads-stale-artifact.md) | 7 | 2 | 3.5 | TODO | [ ] | **Found on the 2026-08-03 prod deploy.** verify-build-lockstep (T6220) compares the site's /build.json against backend /api/version to prove both came from the same commit. After this deploy they were 104 builds apart (frontend 3204/cb90af86 vs backend 3308/bce639d0), and it was NOT a CDN artifact - the DIRECT Pages deployment URL served the same stale values while Cloudflare's newest Production deployment was correctly sourced from bce639d. Root cause: generate-version.js is hooked to `prebuild`, but the deploy runs `build:production`, and npm only auto-runs pre<script> for the MATCHING script name - so production builds never regenerate public/build.json (also gitignored, so a clean checkout can ship it missing, which the verify treats as failure). The shipped bundle is fine (vite.config.js bakes the build number itself via its own git rev-list) so users are unaffected; what is broken is THE CHECK - a guard reading a constant production never refreshes either false-alarms or passes on coincidence. Compounding: on this deploy the verify could not even execute (`Python was not found`) and deploy_production.sh still exited 0, so the drift was caught only by hand. Fix: stamp build.json from a vite hook so every build mode does it (killing the duplicated computation vite.config.js's own comment admits must be kept in agreement), and make an unrunnable verify FAIL the deploy instead of passing it. |
...
| T5830 | ↳ [Heal arshia's already-moved reels](tasks/profile-game-attribution/T5830-heal-arshia-moved-reels.md) | 4 | 3 | 1.3 | DONE | [x] | **DONE (deployed + healed on prod 2026-08-03).** Epic 4/4. Read-only dry run against downloaded copies of his prod DBs -> 14 matched / 0 ambiguous / 7 unmatched, USER-SIGNED-OFF, then shipped as profile_db v033 and executed on prod. Verified in prod R2 (authoritative, not just the API): Maddie U13 3 references / 6 of 9 reels attributed, Ella U13 3 references / 8 of 12, both at schema v33; the owning default profile correctly got 0 references and Maddie 12U / Ella 12U untouched. 6 reference rows total, exactly as signed off. The 7 unmatched stay unattributed forever (6 have no surviving source clip, 1 has a NULL clip_game_start_time) - guessing them could attribute a reel to the wrong game. Rehearsed end-to-end on staging first (his prod account copied there), including an idempotency re-run that reported migrated=0/skipped=3 and changed nothing. Gated on data shape (unique filename x full-precision clip_game_start_time), proven a no-op elsewhere by a sha256 byte-identical fixture test. Branch CI caught the missing T6030 head audit (v033 adds no column) before merge. |
...
### Milestone: Movement Tracking (Activity Timeline for Annotate)
...
### Milestone: Dual-Camera Shared Games
...
### Milestone: SEO & Organic Discovery
