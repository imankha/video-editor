...
| T3610 | ↳ [Collections Tab + Game Collections](tasks/season-highlights/T3610-collections-tab-game-collections.md) | 8 | 6 | 1.3 | TODO | [ ] | My Reels default tab groups reels by game: summary-first data layer (O(games) GROUP BY endpoint, members lazy-loaded on expand), CollectionHeader (ratio pills, verbs slot), touch-first story player (tap/swipe + auto-advance), Mixes & compilations bucket. Mobile pass at 360-428px. |
...
| T3750 | [Rewrite new-user-flow E2E for the new 3-Quest Structure](tasks/T3750-rewrite-new-user-flow-e2e.md) | 7 | 4 | 1.8 | TODO | [ ] | new-user-flow.spec.js drives the OLD flow/step-IDs and old button labels; rewrite to the 3-quest flow (Get Started / Frame Your Highlight / Spotlight Your Player) + new event gestures + Export Highlight/Add Spotlight selectors; Vamos fires after quest_3. Update regression-tests.spec.js label selectors. Coordinate with T3660. |
...
| T1536 | [Quests /progress + /achievements Latency](tasks/T1536-quests-progress-endpoint-latency.md) | 6 | 3 | 2.0 | TODO | [ ] | Prod HAR: `/quests/progress` up to 699ms, `/achievements` up to 636ms (all server `wait`), ~400ms above the ~200ms baseline, fired on most gestures. Root cause: `/progress` opens user.sqlite TWICE + profile.sqlite once per call (R2-restore overhead), NOT `_check_all_steps` (verified: no dead quests/steps post-T3700, 4 cheap queries). Fix: profile-first (attribution), then merge the two user.sqlite opens into one; conditionally skip profile.sqlite for fully-completed users. No persistence-model change. Documents prod log-access gotcha (X-User-ID off in prod). |
| T1537 | [Consolidate Achievement POSTs](tasks/T1537-consolidate-achievement-posts.md) | 5 | 3 | 1.7 | TODO | [ ] | Each crop/speed/color/shape gesture fires 2 POSTs (`/actions` + `/quests/achievements/*`) plus a preflight each. Derive the achievement from the action type server-side and fold the `INSERT OR IGNORE` into the action handler (framing_action reuses its existing profile.sqlite conn; non-blocking). Drop the 4 frontend `recordAchievement` calls; keep lifecycle ones (`opened_*`, `overlay_players_assigned`, gallery views). The "easy cleanup." Pairs with T1536. |
