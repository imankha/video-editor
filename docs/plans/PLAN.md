...
| T7030 | [Intro card image doesn't load; scrub-back leaves video blank](tasks/T7030-intro-card-image-scrub-back-blank.md) | 7 | 5 | 1.4 | WAITING ON USER | [ ] | Filed 2026-08-14 from user report + HAR on staging. `/downloads/{id}/stream` 404s on a retry immediately after the first attempt is client-cancelled — matches "video stops blank" on scrub-back. Separately, no intro-image network request fires at all when scrubbing back into the intro segment, pointing at a stale-mount bug in `CompositeScrubber`/`IntroStoryPlayer`/`MotionPreview` (same component family as T6710/T6730/T6860/T6870). |
