...
| T1516 | [Suppress Export-Job Analytics During Impersonation](tasks/T1516-suppress-export-job-analytics-impersonation.md) | 4 | 2 | 2.0 | TODO | [ ] | Split from T1515 gap: export-completion milestones (export_completed/export_failed) fire from the background export worker, outside the request ContextVar T1515 added, so an export started while impersonating still records on the impersonated user. Stamp an `impersonated` flag on the export job at creation and skip the milestone on completion (design approach a). |
