...
|------|------|------|------|------|------|------|------|
| T10860 | [Update shared version: re-point share token to moved final_video_id after private re-export](tasks/T10860-update-shared-version-repoint-token.md) | 4 | 6 | 0.7 | WIP | [ ] | **Split out of T10180 item 4 by the Architect's design gate, 2026-09-21** (`T10180-design.md` §5, user-approved). The only backend piece of T10180's original scope - a CAS-safe re-point of an existing share token to a NEW `final_video_id` after a private re-export, needing live proof against a real backend that T10180's frontend-only work could not provide. Depends on T10180 shipping first. |
| T10870 | [Tell the user when auto-spotlight falls back to a centered default](tasks/T10870-surface-detection-fallback-message.md) | 4 | 2 | 2.0 | STAGING | [ ] | **Merged PR #518 (bae7809b), 2026-09-26.** Reviewer APPROVED + proof-verifier VERIFIED on the second evidence round (first round caught a real gap in the once-per-region test via mutation testing - `Toast.jsx`'s dedupKey masked the guard's effect on toast count). CI green, landed via `scripts/landing_gate.py`. |
| T10180 | [Result-surface Publish -> visibility-review -> link-ready UI](tasks/T10180-result-publish-visibility-review-link-ready.md) | 6 | 7 | 0.9 | STAGING | [ ] | **Row was stale (still read TODO after merge); corrected 2026-09-25.** T9880 GATED_DISCOVERY follow-up. Merged PR #489, CI green, all ACs provably verified incl. a real Download proof built live on a real never-published draft. Also picks up the dropped T10000 scope (surface existing Download on the private result view). |
| T10190 | [Result-surface title consistency, back-to-game backlink, copy centralization](tasks/T10190-result-title-consistency-backlink-copy.md) | 5 | 5 | 1.0 | STAGING | [ ] | **Row was stale (still read TODO after merge); corrected 2026-09-25.** T9890 GATED_DISCOVERY follow-up. Merged PR #492, CI green. Its `onBackToGame`/`gameId` shaper code lives inside `FocusScreen.jsx` and `OverlayScreen.jsx` completion payloads and `RESULT_SURFACE` copy in `displayNames.js` (already "highlight"-only, no "clip"/"reel") — the Highlight-First and Single-Clip Editor epics touch these same files; see the preserve-notes added to T11240/T11260/T11270/T11280 2026-09-25. |
...
| T11220 | ↳ [Legacy multi-clip data: keep drafts reachable, block re-edit/restore of multi-clip reels](tasks/single-clip-editor/T11220-legacy-multiclip-data-handling.md) | 8 | 5 | 1.6 | WIP | [ ] | No user may lose a draft or published video. Depends on T11200 + R3/R4. Container spawned 2026-09-26 (dotask, 7-task queue T11220-T11280, strict serial). |
...
| T11170 | ↳ [Welcome credits no longer depend on quests](tasks/quest-removal/T11170-welcome-credits-decouple.md) | 8 | 2 | 4.0 | STAGING | [ ] | Merged PR #508 (9cd37f5b), proof VERIFIED, CI green. Grant moved to `storage_credits.grant_welcome_credits`; ledger strings frozen; admin backfill endpoint deleted (never ran on prod). |
