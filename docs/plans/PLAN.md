...
| T6510 | [Rename "Cover photo" to "Preview image" and say what it is for](tasks/T6510-rename-cover-photo-preview-image.md) | 4 | 1 | 4.0 | TODO | [ ] | **User request 2026-08-04.** "Cover photo" reads like an album cover; its actual job is the LINK PREVIEW still shown when a share link is unfurled. Rename the label and add hover copy saying where it appears ("This image is what people see when you share the link"). Include the poster marker's aria-label and tooltip - a screen reader saying "Cover photo marker" against a different visible label is worse than either. LABEL ONLY: no stored field, API key or R2 key renamed. Check tutorial/marketing copy for the old wording. |
