...
| T10890 | [Annotate span-bar readability + playhead nearest-center selection](tasks/T10890-annotate-span-bar-readability-and-playhead-selection.md) | 6 | 3 | 2.0 | STAGING | [ ] | Filed from a prod screenshot (imankh@gmail.com, 88min game, 29 plays): span bars under play markers all render as ~3px dots regardless of clip length. Empirically confirmed H1 (scale, not a bug) via drive-app-as-user against the dev fixture's matching-shape game — percentages ARE proportional, just under the 3px floor on a long game at typical widths; decision artifact to follow before any behavior change. Bundled bug fix (same files, found together): playhead-driven auto-select uses first-array-order match instead of nearest-center when plays overlap. |
| T10900 | ["Confirm Delete" overlaps Cancel/Done in the desktop clip-editor strip](tasks/T10900-confirm-delete-overlaps-done.md) | 5 | 1 | 5.0 | STAGING | [ ] | Prod screenshot 2026-09-21. The strip wrapped DeletePlayButton in a fixed `w-32` slot; the two-button confirm state overflowed it and painted over Done. Slot is now `min-w-[8rem]`, confirm buttons `whitespace-nowrap`. |
| T10910 | [Annotate page scrolls down past all the UI](tasks/T10910-annotate-page-scrolls-past-ui.md) | 6 | 1 | 6.0 | STAGING | [ ] | Prod screenshot 2026-09-21. Browser-verified root cause: `RatingIcon`'s `sr-only` label is position:absolute inside a non-positioned wrapper, so every off-screen clip-list row's icon escaped the `overflow-hidden` shell and grew the document (1239px vs 900px viewport). Wrapper is now `relative`; re-probed at 900 == viewport. |
| T10920 | [Review plays: new icon + published/clipped mark on the play banner](tasks/T10920-review-plays-icon-and-clip-state-mark.md) | 5 | 2 | 2.5 | STAGING | [ ] | User request 2026-09-21. `ListVideo` icon for the playback badge + both Review plays buttons; `NotesOverlay` gets a `clipStage` and draws a green check (PUBLISHED) or a cyan film glyph (clip exists, not published) in its upper-right corner, via the same `getClipStage` lookup the strip CTA uses. |
| T10930 | [Annotate timeline zoom in/out with a visible control, desktop and mobile](tasks/T10930-annotate-timeline-zoom-controls.md) | 7 | 4 | 1.8 | STAGING | [ ] | User request 2026-09-21; approved same day from the decision artifact https://claude.ai/artifact/VRztYxsiraMJz6wsMrUENB (option D + chip on both). Shipped: `-  N%  +` chip rendered by TimelineBase (Annotate, Focus and Overlay), AnnotateModeView owns useTimelineZoom so fullscreen toggles keep the zoom, phone opens at 300% and can go 100-500%, wheel + scroll pill unchanged, no pinch. Live-verified on the fixture 88-min game: 26 s play 10.5 px vs 10 s play 4 px at 300%. |
| T10940 | [Silent app update did not fire; user had to hard-refresh](tasks/T10940-silent-update-did-not-fire-hard-refresh-needed.md) | 8 | 4 | 2.0 | STAGING | [ ] | PROD incident 2026-09-21 (confirmed by the user: old UI after the deploy, plain refresh still old, only Ctrl+refresh worked). Root cause, code-verified and reproduced in a real browser: workbox-window registers only after window `load`, so the on-load `/api/version` probe ran against a NULL registration, answered "no bundle" and locked the probe out for the full 5 min on EVERY load of the stale shell (a plain reload repeated the race). Fix: the probe awaits the registration (15s bound) and there is ONE 25s probe cooldown (< the 30s poll), replacing T9310 Gap C. Real-SW e2e case 5 = the exact sequence: red on the old code, green in 31s. No popup, quiescence gate untouched. |
| T10960 | [Trimming one handle in the clip editor threw the other handle off the track](tasks/T10960-trim-end-handle-off-track-after-start-trim.md) | 6 | 1 | 6.0 | STAGING | [ ] | PROD bug 2026-09-21 (imankh, clip editor): drag START in, on save the END lever vanished off the right edge. Root cause: the edit-mode zoom window's half-width re-derived from the LIVE saved clip (new `existingClip` object per commit, T10410) while its anchor stayed frozen per clip id at the OLD midpoint, so the narrower window no longer contained the untouched handle (rendered at 117% left). Fix: edit-mode anchor = live clip midpoint (same snapshot as the half-width); create mode keeps the frozen ref. Unit test replays the prod trim, red without the fix. |
