...
|------|------|------|------|------|------|------|------|
|  | **[Draft Stage Display](tasks/draft-stage-display/EPIC.md)** | 6 | 4 | 1.5 |  |  | Filed 2026-08-11 from direct user feedback during the code freeze. Reel Drafts inside a game group should read at a glance: Not Started tiles currently render as portrait 9:16 shells (target aspect) center-cropping a landscape source-aspect poster; stages are only visible via per-tile chips; un-started drafts get no hover preview. T6800+T6810 are frontend view-only and wanted INSIDE the current freeze build; T6820 (backend payload + preview player) lands after. Stage derivation + row rules in EPIC.md. |
| T6820 | ↳ [Hover preview for Not Started drafts (source clip window)](tasks/draft-stage-display/T6820-hover-preview-not-started-source-clip.md) | 5 | 4 | 1.25 | STAGING | [ ] | Extends the T6420 preview chain (final → working → source clip window) via the bounded clip-window proxy. Follow-up (user report 2026-08-14): the source-clip tier's hover reveal felt slow — generalized the reveal-delay policy across ALL preview tiers to max(the ~450ms floor, real content-load-ready time) instead of a blind fixed timer, so a slow-loading tier never pays floor-plus-load-time back to back. |
| T5140 | [Reshoot Tutorial Videos (carousel/poster UI + Team layer; slow narration baked in, 1x playback)](tasks/T5140-reshoot-tutorial-videos.md) | 7 | 4 | 1.8 | TODO | [ ] | Closes the UI-visible runway — reshoot ALL 4 quest tutorials once it has landed. The 2026-07-18 recordings predate the poster-tile home (T5672/T5673/T5681), the Team / My Athlete layer + Team-only teammate tagging (T5700/T5725/T6400), the Google-Docs share modal + public game link (T5720/T5740), export cost on the button (T5780/T5790), the looping "Play spotlight" control (T5370), and the My Reels tile/CTA rework (T5678/T6180/T6300) — every quest shows dead UI. Also bakes the slow pace into the AUDIO (tts rate ~ -10%) so playback defaults to 1x: after upload, flip DEFAULT_RATE 0.8 -> 1 in TutorialVideoModal.jsx + landing TutorialModal.tsx. Work happens in C:/Users/imank/Videos/Captures/ReelBallersTutroials (read its CLAUDE.md first); the task file carries the per-quest drift table, recommended talk-track edits, and the capture-spec triage list. |
|  | **[Collection Download](tasks/collection-download/EPIC.md)** | 6 | 5 | 1.2 |  |  | Sequenced after T5140 (user-ordered 2026-08-10) — not a UI-runway member, so it doesn't need to land before the reshoot. From T3950 pivot (2026-07-12): user wants compilations burned on download, but NO collection download exists - collections are composited playback only. Design pass 2026-08-10 found the mechanism is mostly wiring (reuses `compose_serve_time` + `concat_segments` unchanged) - split into 3 children after user review widened scope beyond the original single-task design. Recovery/job-backed robustness split to its own epic, [collection-download-recovery](tasks/collection-download-recovery/EPIC.md) - both parked (docs only) per user decision 2026-08-10, not in active implementation. |
| T4945 | ↳ [Core stitch + owner download](tasks/collection-download/T4945-core-stitch-owner-download.md) | 6 | 5 | 1.2 | STAGING | [x] | Stitch member reels in rank order into one MP4 ending with exactly one branded outro; compute location config-gated on the existing `MODAL_ENABLED` flag (local ffmpeg in dev, Modal-routed in prod - protects the single shared app server from concat/re-encode CPU load). Mixed-resolution via `concat_segments`' built-in fallback. See EPIC.md for the full resolved-decisions table. |
| T4946 | ↳ [Access control (permission + sign-in, free)](tasks/collection-download/T4946-access-control-and-credits.md) | 6 | 2 | 3.0 | TODO | [ ] | **Unblocked 2026-08-14 — Decision 4 resolved free.** Margin analysis (credit pricing + CPU-only Modal stitch cost + zero R2 egress) showed free collection downloads cost well under 1pp of margin even at heavy download volume. Scope now: investigate the collection/sharing permission model (owner-only today? real collaborator concept?) and gate T4945's endpoint on permission + sign-in only — no credit machinery. |
| T4947 | ↳ [Cache stitched downloads](tasks/collection-download/T4947-cache-stitched-downloads.md) | 5 | 2 | 2.5 | TODO | [ ] | Disposable R2 cache keyed by member-set hash (no DB row, no migration). Credit-recharge-exemption half of the original scope is moot (downloads are free, Decision 4). Depends on T4946. |

All near/mid-term work that **visibly changes the UI** lands first — including [T6630](tasks/player-intro/T6630-overlay-text-add-remove-drag-ux.md) (now STAGING, merged 2026-08-08) — and the tutorial reshoot ([T5140](tasks/T5140-reshoot-tutorial-videos.md)) closes that runway so the videos capture settled screens. Deliberately NOT gating the reshoot: [T6500](tasks/T6500-overlay-font-catalogue.md) (pushed out 2026-08-09, polish-only), T5750 (evidence-gated), Multi-File Ingest & Prep (deprioritized 2026-07-31), Movement Tracking + Dual-Camera milestones (post-reshoot features — when one lands, the affected quest gets a touch-up reshoot, not a full redo), the UI Pass epic row (all children shipped; row open only for ledger reconciliation), and the [Collection Download](tasks/collection-download/EPIC.md) epic (sequenced after T5140, user-ordered 2026-08-10 — see its row above). **Exception (user-ordered 2026-08-10):** [T6350](tasks/T6350-move-reels-half-apply-on-sync-failure.md)/[T6345](tasks/T6345-postgres-migration-runner-skips-version-gaps.md)/[T6410](tasks/T6410-migration-swap-discards-unsynced-writes.md) were refactor/durability work with no visible UI change that landed BEFORE the reshoot rather than after — shipped in the 2026-08-13 prod deploy, archived to [PLAN-archive.md](PLAN-archive.md) (Milestone TOP section). [T6441](tasks/tile-video-preview/T6441-hover-preview-in-overlay-drafts.md) also moved ahead of T5140 (bundled into the same pre-reshoot session, small enough to not delay it) and is likewise archived. **Updated 2026-08-09:** T6680/T6690/T6700 (all UI-visible) also gated the reshoot and are now archived (2026-08-13 prod deploy); T6710 (design-gated, not yet built at the time) is lower-risk to slip since tutorials don't have to demo intro-card playback specifically — the Overlay quest's text-editor rail work (T6480/T6600/T6610/T6630/T6640) was on-screen in the current tutorial and has since landed too (archived, 2026-08-13 prod deploy). If Player Intro slips badly, drag T5140 above it on the board.

**Queued directly after the reshoot (user-ordered 2026-08-04):** the [JIT Migration epic](tasks/jit-migration/EPIC.md) (T5081-T5089). Not a UI-runway member — it sits here because the user placed it immediately after T5140 and ahead of Tile Video Preview. [Collection Download](tasks/collection-download/EPIC.md) (above) is a second member of this same "after the reshoot" group — relative order between the two not yet specified by the user.

|  | **[Tile Video Preview](tasks/tile-video-preview/EPIC.md)** | 6 | 4 | 1.5 |  |  | Design settled 2026-08-03, filed as a fully-specced epic but never given a PLAN.md row until 2026-08-10 (found untracked while researching a user request — was previously only mentioned in prose above). Draft/reel tiles play their video inline (muted, looping, in place) on glance instead of a static poster — desktop hover, touch in-viewport autoplay, both against YouTube/Netflix prior art (design authority table in EPIC.md). Deliberately placed AFTER the T5140 reshoot (user decision) so recordings aren't invalidated by a feature that's simply absent from them. Not started. |
| T6430 | ↳ [Touch: in-viewport autoplay](tasks/tile-video-preview/T6430-touch-in-viewport-autoplay.md) | 5 | 4 | 1.25 | TODO | [ ] | Epic child 2/3, depends on T6420 (reuses `TilePreviewVideo` + the single-active registry). In-viewport autoplay of the single most-visible tile while scrolling (YouTube/Netflix/Instagram precedent) - deliberately NOT long-press, which would be exactly the hidden-gesture discoverability failure T6300 already documented on these same tiles. Leaves the existing long-press = reveal-actions gesture untouched. |
| T6440 | ↳ [Autoplay-previews setting + data-saver](tasks/tile-video-preview/T6440-autoplay-setting-data-saver.md) | 3 | 2 | 1.5 | TODO | [ ] | Epic child 3/3, depends on T6420 (+T6430 for touch). "Autoplay previews" toggle (default ON) in the existing settings surface, persisted gesture-based - Netflix shipped without this and had to add it under user pressure. Data-saver guardrail for metered connections (sideline cellular is exactly this audience). |
...
| T6500 | [A font catalogue for overlay text](tasks/T6500-overlay-font-catalogue.md) | 6 | 4 | 1.5 | TODO | [ ] | **Pushed out 2026-08-09 — no longer epic-tracked, not gating anything, polish-only.** Nothing is broken today (the existing faces render and function); this is whether they read *well* at small burned-in scale, purely deferrable. **User request 2026-08-04.** The 4 faces (anton/oswald/graduate/playfair) were chosen for INTRO CARDS - full-frame, mostly-caps display. Overlay text is short callouts burned over live footage, read at a glance, often small on a busy background. Main design question: split the catalogue by context or grow one list - decide against RENDERED samples, not prose. T5180's standing note said add a neutral sans "THEN, not speculatively" if small print reads badly in real use; overlay use is that THEN. Both sides must ship each face (parity test is the proof). Check lowercase + a long string - neither has ever been tested. |
| T6780 | [Guard-asymmetry follow-up: detections_data write + games materialization writes](tasks/T6780-guard-asymmetries-detections-materialization.md) | 5 | 4 | 1.3 | TODO | [ ] | **Filed 2026-08-11 by the T6550 worker's sibling-asymmetry sweep.** Two guarded-read/unguarded-write asymmetries on render/materialize paths (not simple gesture writes, so not mirrored inline by T6550): (A) `working_videos.detections_data` (v027) guarded read vs unguarded `upsert_working_video` INSERT+UPDATE - reachability against a below-v027 profile not yet established. (B) `games.shared_by`(v026)/`source_profile_id`+`source_game_id`(v030) guarded reads vs unguarded `materialization.py` copy-game INSERT into a possibly-below-head RECIPIENT profile. Each needs its own design call before a fix shape is chosen. |
| T6760 | [E2E suite hygiene - stop rediscovering the same rot every sweep](tasks/T6760-e2e-suite-hygiene.md) | 5 | 3 | 1.5 | TODO | [ ] | **Filed 2026-08-11, Part C of the derisk plan.** The 2026-08-11 full e2e sweep (536 tests, 342 pass / 125 fail) root-caused every failure as non-product-regression noise falling into 3 classes: real-account specs that hang the full 5-min timeout on stale fixture data instead of failing fast, specs that require a precursor QA-harness step and fail-by-design with no explanation, and per-task round-N evidence specs that rotted after their target UI was superseded. Fast-fail account-status guard + explicit skip-guard pattern (bug27p precedent) + archive/prune superseded evidence specs. Full 57-file failed-spec list is in the task file; NOT yet triaged file-by-file - that's step 1. |
| T6770 | [Replace game_ref_counts.ref_count with a derived ref-set](tasks/T6770-game-refcount-derived-set.md) | 6 | 5 | 1.3 | TODO | [ ] | **Filed 2026-08-11, Part C of the derisk plan** - formalizes the still-open follow-up from `.claude/retrospectives/game-video-refcount-drift-incident.md` ("replace the bare counter with a derived ref-set... larger change, follow-up task"). New evidence 2026-08-11: dev Postgres has NO `game_ref_counts` row at all for imankh's games 2/3/5 while their per-profile `game_storage` rows are active - a different drift shape (missing vs. negative) than the 2026-07-24 incident, same underlying class. Postgres schema change (new table + backfill migration); must not regress the existing sweep/live-ref-guard tests (T5850's authoritative-recount logic especially). |
...
| T7000 | [`_js_path()` loud-failure guard no longer raises at Fly deploy depth](tasks/T7000-js-path-guard-broken-fly-depth.md) | 4 | 2 | 2.0 | TODO | [ ] | Filed 2026-08-13 while triaging T6990's Branch CI. `test_t6920_intro_geometry_import_depth.py::test_js_path_raises_clear_error_at_fly_image_depth` fails on Master CI itself (unrelated to T6990's diff) — the T6920 no-silent-fallbacks guard no longer raises at the mocked Fly deploy depth. No commit has touched `intro_card_geometry.py` since T6920 landed, so likely an environment/import-caching difference on the CI runner rather than a code regression; needs root-causing. Listed in `docs/testing/known-failures.md` in the meantime. |
| T7040 | [Collection download fails with "TypeError: Failed to fetch"](tasks/T7040-collection-download-failed-to-fetch.md) | 8 | 5 | 1.6 | TODO | [ ] | Filed 2026-08-14 from user report on staging — T4945's just-merged endpoint failed its first real use. CORS preflight succeeds cleanly (200, 20.5ms) but the actual GET never resolves; leading hypothesis is a mid-stream exception after the StreamingResponse's headers are already committed (Modal cold-start-plus-concat is also a candidate, since `stitch_members` genuinely exists and hydrates on the live Modal app — ruled out as never-deployed). Needs live server-log tailing during a real repro, not retrospective log capture. |
| T7030 | [Intro card image doesn't load; scrub-back leaves video blank](tasks/T7030-intro-card-image-scrub-back-blank.md) | 7 | 5 | 1.4 | TODO | [ ] | Filed 2026-08-14 from user report + HAR on staging. `/downloads/{id}/stream` 404s on a retry immediately after the first attempt is client-cancelled — matches "video stops blank" on scrub-back. Separately, no intro-image network request fires at all when scrubbing back into the intro segment, pointing at a stale-mount bug in `CompositeScrubber`/`IntroStoryPlayer`/`MotionPreview` (same component family as T6710/T6730/T6860/T6870). |
| T7020 | [Remux uploaded game videos with faststart (moov at front)](tasks/T7020-game-video-faststart-remux.md) | 6 | 4 | 1.5 | TODO | [ ] | Filed 2026-08-14 while investigating T6820's ~4s hover-preview delay. Real R2 trace showed 5 sequential round trips per seek because uploaded game videos' `moov` atom sits at the END of the file (device-written, never remuxed). Lossless `-c copy -movflags +faststart` at upload finalize (mirroring the pattern already used in `ai_upscaler/video_encoder.py` and Modal export functions) would collapse this to 1-2 round trips. New uploads only, no backfill of existing videos. |
| T7010 | [Clip-save "couldn't save" error + both clips landed under the same game (needs better logging)](tasks/T7010-clip-save-conflict-and-game-misattribution.md) | 6 | 4 | 1.5 | STAGING | [ ] | DONE 2026-08-14: resolved as no bug. The save error was a real CAS `SYNC_CONFLICT`/`stale_baseline` (known multi-container-QA-one-account collision, several `/dotask` stacks live simultaneously) that auto-healed via retry, no data lost. The game-grouping "mystery" resolved once the user clarified they were promoting old previously-rated clips (both genuinely from the same 2026-06-11 annotate session on game 6, 22 min apart in match time) into reels, not annotating live — confirmed directly against the profile DB. Logging added regardless: frontend-believed game id alongside backend-read game id at save time, CRITICAL log on a mid-write DB heal. |
