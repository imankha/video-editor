...
| ID | Task | Impact | Cmplx | Pri | Status | Migr | Description |
|------|------|------|------|------|------|------|------|
|  | **[Durable Sync — No Silent Data Loss](tasks/durability-sync/EPIC.md)** | 9 | 5 | 1.8 | TODO |  | TOP. A committed write must never be silently destroyed by a machine swap, an R2 error, or a stale local snapshot force-pushing over newer cloud state. T4310 (CAS upload) + T4315 (restore-if-newer) interlock; T4320 done. Narrowed 2026-07-24 — format/concurrency tasks moved to Write Correctness. |
...
| T5830 | ↳ [Heal arshia's already-moved reels](tasks/profile-game-attribution/T5830-heal-arshia-moved-reels.md) | 4 | 3 | 1.3 | WIP | [x] | **2026-07-31: NOT STARTED - hard-blocked, deliberately not spawned.** Two gates must clear first: (1) T5800+T5810 merged AND DEPLOYED TO PROD (the heal writes T5810-shaped state, so the primitive must exist there), and (2) a dry-run mapping table produced from arshia's downloaded prod R2 profile DBs with explicit user sign-off BEFORE any write (data-safety rule). Also re-derive his kid-profile ids from prod user.sqlite at dry-run time - the T5310-era ids 6ff007e6/22c7616a are stale, do not trust them. Note the heal version is head+1 at implementation time, i.e. **v031** now that T5800 claims v030. Epic 4/4, after 1-3 are on prod. His pre-fix moved reels have game_ids NULL + source rows deleted; "Assign to game" gesture deferred (user), so heal directly: match single-clip reels via clip_game_start_time == default-profile raw_clips.start_time + name/tags (UNAMBIGUOUS matches only, never guess), write T5810-shaped state (reference + game_ids) via a gated versioned profile_db migration (v012/v018/v019 precedent, no-op for everyone else). Dry-run on downloaded R2 DBs + user sign-off on the exact mapping BEFORE any write. **UPDATE 2026-07-24: the 5 clobber-lost final_videos rows are ALREADY RECOVERED** (re-inserted from authoritative source data — see [[project_arshia_lost_reels_move_clobber]]); this task is now ONLY the game_ids attribution for his moved reels, which now includes those 5. **File contention: T5810 edits the SAME `move_reels_to_profile` that got the durability fix (a5ff3e48 `require_fresh`) — rebase on it, don't revert it.** |
|  | **[UI Pass (home tiles/carousel + polish)](tasks/ui-pass/EPIC.md)** | 6 | 4 | 1.5 | TODO |  | User direction 2026-07-20: no poor/ugly/wrong-for-the-problem UI, nothing drastic; tiles > list for drafts, "Netflix style carousel under the game". Grounded in a real-browser audit (desktop 1315px + mobile 390px, imankh dev data) + code map: drafts/games are text-only rows in one max-w-2xl column inside 2271-line ProjectManager.jsx; NO thumbnail field exists anywhere (posters exist only for published reels, T5280); no carousel primitive in the codebase. 13-finding ledger in EPIC.md (collisions, overflow, pillarboxing, cryptic labels, dead deep-links). Every task ships mobile (360-428) AND desktop, real-browser verified. T5671->72->73 sequenced; T5674-77 independent. T5672 and T5675 both touch ProjectManager.jsx — never run concurrently. |
|  | **[Share the Game (team layer + public game link)](tasks/team-game-share/EPIC.md)** | 9 | 7 | 1.3 |  |  | Consolidates T4910 + T4920 (both superseded) + recap separation, per 2026-07-21 design session. Parent tags TEAM plays alongside My Athlete plays (layer = existing `raw_clips.my_athlete` bit — no migration), recaps split per layer, one public link in the team WhatsApp plays the team recap instantly (no signup wall), and viewers claim the game + optional team annotations into their own account (T5330 provenance preserved). Watch-first growth loop into the other ~15 families per team. Locked decisions in EPIC.md: strictly one layer/clip, anonymous = team recap only, combined recap replaced, links live until revoked. Coordinates token/claim plumbing with Dual-Camera T5500 (first to land owns it). |
...
| ID | Task | Impact | Cmplx | Pri | Status | Migr | Description |
|------|------|------|------|------|------|------|------|
|  | **[Durable Sync — No Silent Data Loss](tasks/durability-sync/EPIC.md)** | 9 | 5 | 1.8 | TODO |  | PROMOTED to the TOP milestone (see "Milestone TOP: Durable Sync" above). T4310 + T4315 + sibling T5840 live there. This row is a pointer only — do not duplicate the rows here. |
