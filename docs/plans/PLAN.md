...
| T4000 | [Parallelize Game Video Fetch With /load](tasks/T4000-parallelize-game-video-fetch.md) | 6 | 4 | 1.5 | TODO | [ ] | Opening a game is two sequential round-trips: `/api/games/{id}/load` returns the presigned `playback_url`, and only then does the `<video>` start fetching bytes (~762ms total). Diagnosed from prod HAR: backend is NOT slow and it's NOT a Fly wake (both ruled out via live timing + machine event log) - it's a chained dependency. Start the video fetch in parallel with `/load` using a stable gameId-only URL (`/stream` proxy or `/video` 302->R2; design-gated). Expected ~762ms -> ~450ms. Frontend-mostly; no reactive persistence. |

### Bugs

| ID | Task | Impact | Cmplx | Pri | Status | Migr | Description |
|------|------|------|------|------|------|------|------|
| T3990 | [First Reel-Draft Click Refreshes the App](tasks/T3990-first-draft-click-page-refresh.md) | 6 | 3 | 2.0 | TODO | [ ] | First click on a reel draft after the tab has been open across a deploy does a full-page refresh instead of opening it (works after). Root cause (HAR-confirmed): lazy `import()` of the editor chunk hits a purged old-build hash, server returns index.html (text/html), `import()` rejects, `lazyWithReload` reloads. Fix: eagerly preload editor chunks on home-screen idle so the first click reuses the cached module; keep lazyWithReload+breadcrumb as fallback. Frontend-only. |
