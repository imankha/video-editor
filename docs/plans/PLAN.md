...
**Now unblocked — the "after the reshoot" group is ready to start:** [T7140](tasks/T7140-game-video-faststart-remux.md) (user-ordered 2026-08-15, renumbered from T7020 on 2026-08-17) is FIRST — see its row above for why it's deliberately held rather than shipped as-is. Then the [JIT Migration epic](tasks/jit-migration/EPIC.md) (T5081-T5089, user-ordered 2026-08-04) — not a UI-runway member, placed immediately after T5140 and ahead of Preview Video Improvements. [Collection Download](tasks/collection-download/EPIC.md) (above) is complete already. Relative order between JIT Migration and Preview Video Improvements not yet specified by the user. **The [SEO Content & Landing Value Props epic](tasks/seo-content/EPIC.md) is NOT part of this group** — per the Priority Policy (growth work sits behind infrastructure bugs) it is sequenced after Milestone TOP finishes entirely, not merely after the reshoot; see the SEO & Organic Discovery section below.
...
| T7250 | ↳ [Unsubscribe endpoint + compliance footer](tasks/lifecycle-drip/T7250-unsubscribe-compliance.md) | 7 | 3 | 2.3 | TODO | [ ] | Epic 3/5, MUST precede any live send. HMAC-tokened public GET /api/email/unsubscribe (no login, idempotent, constant-time verify) writing email_unsubscribes; new _build_drip_email shell (clones _build_update_email + unsubscribe link + physical-address line) + send_drip_email (Resend, List-Unsubscribe header, dev log-only mode). OPEN: COMPANY_MAILING_ADDRESS env value from user (CAN-SPAM). Admin bulk-email shell untouched. |
| T7260 | ↳ [Drip scheduler + idempotent send pipeline + admin dry-run](tasks/lifecycle-drip/T7260-drip-scheduler-send-pipeline.md) | 8 | 5 | 1.6 | TODO | [ ] | Epic 4/5. services/drip_scheduler.py modeled on sweep_scheduler (lifespan start/stop, 6h tick, to_thread), gated DRIP_EMAILS_ENABLED. run_drip_tick(now, dry_run): window query off users.created_at (LEFT JOIN segments — segmentless users still drip) → suppression chain (test patterns, DRIP_SUPPRESSED_EMAILS, is_admin, unsubscribed) → stage → template → CLAIM (ON CONFLICT DO NOTHING RETURNING) → render+send → sent/failed (failed keeps claim, CRITICAL log, no auto-retry). POST /api/admin/drip/run {dry_run} + GET /api/admin/drip/sends. Claim race + zero-catch-up-blast + dry-run-writes-nothing all DB-proven by tests. |
| T7270 | ↳ [Admin panel: template editor + send log](tasks/lifecycle-drip/T7270-admin-template-editor.md) | 6 | 4 | 1.5 | TODO | [ ] | Epic 5/5. The "easy to update" deliverable: 4×7 template grid (one glance = who gets what when), editor modal mirroring BulkEmailModal (char caps, Send-test-to-me with sample vars + real unsubscribe footer, explicit-save gesture, no backdrop close), send log + "Preview next tick" dry-run view. CRUD endpoints thin; enabled=false is the only off switch (no delete/create — cells are the fixed universe). Then run the EPIC rollout sequence. |
