...
| ID | Task | Impact | Cmplx | Pri | Status | Migr | Description |
|------|------|------|------|------|------|------|------|
| T7220 | [Multi-clip temp-source cleanup awaited a sync function](tasks/T7220-multiclip-temp-cleanup-await-bug.md) | 2 | 1 | 2.0 | WIP | [ ] | Found live in staging logs while verifying T7210: `multi_clip.py` awaited `delete_from_r2` (a plain sync function everywhere else in the codebase), throwing `TypeError` on its bool return every export and logging a false "failed to delete" even though the R2 delete had already succeeded. Existing tests mocked it as `async` specifically to mask the mismatch. Fixed with `asyncio.to_thread` (matching the pattern used a few lines earlier for `upload_bytes_to_r2`); test mocks corrected to match the real sync contract; added a regression assertion verified to fail on the reverted code. |
