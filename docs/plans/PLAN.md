...
| T7020 | [Remux uploaded game videos with faststart (moov at front)](tasks/T7020-game-video-faststart-remux.md) | 6 | 4 | 1.5 | WIP | [ ] | Filed 2026-08-14 while investigating T6820's ~4s hover-preview delay. Real R2 trace showed 5 sequential round trips per seek because uploaded game videos' `moov` atom sits at the END of the file (device-written, never remuxed). Lossless `-c copy -movflags +faststart` at upload finalize (mirroring the pattern already used in `ai_upscaler/video_encoder.py` and Modal export functions) would collapse this to 1-2 round trips. New uploads only, no backfill of existing videos. |
