...
| T10980 | [Focus clip rail "Frame clip" badge; aspect-ratio selector reads the project, not a stale copy](tasks/T10980-focus-rail-frame-badge-aspect-ratio-single-source.md) | 5 | 2 | 2.5 | STAGING | [ ] | STAGING (merged 2026-09-21, Branch CI green). Prod screenshot 2026-09-21. (1) The rail's unframed state was a faded gray crop icon (the green disc was the rating); it is now the shared amber "Frame clip" / green "Framed" progress disc. (2) The Aspect ratio selector read `projectDataStore.aspectRatio`, a second copy written only on the Drafts open path, so Focus entered from Annotate showed the previous project's ratio while the crop box drew the real one, and clicking the right button was a no-op. The copy is deleted; `useClipManager.globalAspectRatio` derives from `selectedProject.aspect_ratio`. |
