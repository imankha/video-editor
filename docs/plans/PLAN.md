...
| T10970 | [Overlay Text lane behind a disclosure; "Export" CTA renamed "Apply Overlay"](tasks/T10970-overlay-text-lane-disclosure-apply-overlay.md) | 3 | 1 | 3.0 | STAGING | [ ] | STAGING (merged 2026-09-21, Branch CI green on re-run of the documented t6200 SQLite-lock flake). User request 2026-09-21: the Overlay timeline's Text lane starts hidden behind a `Text` chevron under the timeline, same shape as Focus's Trim and Slo-mo disclosure (T9950): ephemeral override on a derived default, open when the clip already has text regions. `OverlayMode.showTextLane` gates label + track + playhead-line height. Second half: `EXPORT_JOBS.overlay.action` "Export" -> "Apply Overlay" (also the `export_overlay` quest step title, mirrored in backend `quest_config.py`). |
<<<<<<< HEAD
| T11000 | [Framing re-export before picking a player dropped all tracking (0 regions)](tasks/T11000-reexport-before-player-pick-drops-detection.md) | 8 | 1 | 8.0 | STAGING | [ ] | STAGING (pushed to master 2026-09-21, hotfix). PROD 2026-09-21 (imankh, "Great Moves and Pass"): second Framing export came back with zero highlight regions and no tracking markers. Root cause: the prior working video's only region was the detection SEED with `keyframes: []` (no player picked yet); the T4350 carry transform drops keyframe-less regions (`to_raw` returns None) and never re-seeds from the fresh detection. Fix: Rule 1 now seeds from detection whenever NO prior region has a keyframe. 37 carry tests pass incl. a replay of the prod shape. |
=======
| T10980 | [Focus clip rail "Frame clip" badge; aspect-ratio selector reads the project, not a stale copy](tasks/T10980-focus-rail-frame-badge-aspect-ratio-single-source.md) | 5 | 2 | 2.5 | WIP | [ ] | Prod screenshot 2026-09-21. (1) The rail's unframed state was a faded gray crop icon (the green disc was the rating); it is now the shared amber "Frame clip" / green "Framed" progress disc. (2) The Aspect ratio selector read `projectDataStore.aspectRatio`, a second copy written only on the Drafts open path, so Focus entered from Annotate showed the previous project's ratio while the crop box drew the real one, and clicking the right button was a no-op. The copy is deleted; `useClipManager.globalAspectRatio` derives from `selectedProject.aspect_ratio`. |
...
>>>>>>> feature/T10980-focus-rail-frame-badge-aspect-ratio-single-source
