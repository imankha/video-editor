...
| T4110 | [Edit-reel re-export/publish durability + proj-41 heal migration](tasks/T4110-edit-reel-persistence-durability.md) | 9 | 3 | 3.0 | IN PROGRESS | [x] | Re-export/publish of an edited reel silently lost on prod (export finalize not hardened by T4050; rows ride fire-and-forget sync, lost on machine cycle). Fix = sync-then-announce durable export boundary (+ retryable `sync_failed`/503 UX). v018 targeted migration re-publishes+archives imankh project 41. Branch pushed `feature/T4110-edit-reel-persistence-durability`; run profile_db migration after deploy. |
