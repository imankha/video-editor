...
| T3770 | [Confirm StrictMode Duplicate Page-Load Fetches](tasks/T3770-strictmode-duplicate-pageload-fetches.md) | 3 | 2 | 1.5 | TODO | [ ] | **MEASURED (2026-06-18):** bootstrap ×2->×1 and health ×2->×1 are StrictMode-only no-ops (health: ServerStatus is dead code; sole checker is ConnectionStatus). projects/{id} ×3->×2: one StrictMode dup collapsed, but a real ×2 residual remains (projectsStore.fetchProject + ProjectContext both fetch same id = redundant state). Carved into T3775; no code changed here. |
| T3775 | [Eliminate ProjectContext Redundant Project Fetch](tasks/T3775-projectcontext-redundant-project-fetch.md) | 3 | 2 | 1.5 | TODO | [ ] | Follow-up from T3770. ProjectContext re-fetches /api/projects/{id} (bare) on project-open even though projectsStore.selectedProject already holds it (fetched with ?_t) = duplicate request + redundant state. Make ProjectContext consume projectsStore instead of fetching; projects/{id} -> ×1. T2500 promise-guard does NOT apply (different URLs/modules). |
| T3778 | [Remove Dead ServerStatus Component](tasks/T3778-remove-dead-serverstatus-component.md) | 1 | 1 | 1.0 | TODO | [ ] | Follow-up from T3770. components/shared/ServerStatus.jsx is dead code (mounted nowhere; index.js comment says "ServerStatus removed" but the file was never deleted). Pure deletion; confirm no imports, build passes. |
