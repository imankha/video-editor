...
| T8675 | ↳ [Dispute webhook writes ledger rows](tasks/revenue-integrity/T8675-dispute-webhook-ledger-rows.md) | 4 | 3 | 1.3 | WIP | [ ] | Follow-up split from T8620 design gate (user ruling 2026-09-24): no live dispute webhook exists, so a newly lost dispute leaves the ledger above Stripe net until a backfill re-run. Handle the Stripe dispute event, write a negative `dispute_lost` row idempotently, move the cache only on a new row. Depends on T8620. |
