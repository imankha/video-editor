...
| T5210 | ↳ [Card render engine](tasks/player-intro/T5210-intro-card-generation.md) | 7 | 6 | 1.2 | TODO | [ ] | Wave 3. app/services/player_intro.py mirroring branded_outro.py: card row + PNG text layers -> animated MP4, probe-matched, per-format cache, non-fatal. Motion + professionalism IS the deliverable (user direction) - a static frame is not acceptable. Pitch/position diagram DROPPED. |
...
| T6490 | [Pause during overlay text - freeze the frame through a text block](tasks/T6490-pause-during-overlay-text.md) | 7 | 6 | 1.2 | TODO | [ ] | **User request 2026-08-04**, driving T5225 on staging: a per-block option that holds the frame at the block's startTime for the block's duration so a callout can actually be READ, instead of the footage running underneath. Default OFF. The hard part is that a freeze INSERTS TIME the working video does not have - audit every consumer of output duration (poster/slow-mo resolution, _validate_concat, progress, share playback, T5220 intro prepend) and decide render-time expansion vs timeline edit (former almost certainly right). Both render loops must agree or the two paths produce different videos. Architect gate; Modal redeploy is the user's step. |
