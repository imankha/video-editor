...
| T4150 | [PWA In-Session Update](tasks/T4150-pwa-in-session-update.md) | 5 | 2 | 2.5 | TODO | [ ] | Installed PWA never applies a new build in-session: sw.js auto-updates in the background but the app never imports `virtual:pwa-register`, so a long-lived PWA runs a stale bundle until its next full load (caused the false-alarm "unframed 16:9 clips in ranking" prod debug session). Fix: switch VitePWA to `registerType: 'prompt'`, call `registerSW({ onNeedRefresh })` from the app entry to show a persistent "New version available - Refresh" toast wired to `updateSW(true)` (reuses shared Toast, deduped), plus a visibility-triggered `registration.update()` re-check (rate-limited; re-surfaces a dismissed prompt) for long-lived sessions - chosen over setInterval, which browsers freeze in backgrounded PWAs. Frontend-only. |
