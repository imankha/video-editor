...
| T4140 | [Recap as Full-Quality Re-Edit Source (Create Clip survives game expiry)](tasks/T4140-recap-as-reedit-source.md) | 7 | 7 | 1.0 | TODO | [ ] | Make the surviving recap (`recaps/{game_id}.mp4`) a real re-edit source so T4130's "+Create Clip" (and edit-reel re-export) keeps working after the game video is swept. Today the recap is a 480p/crf32 review proxy and re-export only sources from the deleted game video. Decisions: single upgraded recap (one file for playback/share/re-edit), **original resolution** + master-grade encode, **reframe-only** (frozen annotated bounds, no wider trims). Work: hi-q `_generate_recap`; a `resolve_clip_source` helper (prefer game video, else recap at `recap_start/recap_end`, else fail) wired into the reframe/export path; throttled backfill to regenerate hi-q recaps for not-yet-deleted games; storage-credits accounting. Blocking design check: crop-keyframe validity vs concat resolution. Follows T4130. Architecture-gated. |
...
| T3980 | [Dev-Login: Faithful Account Impersonation](tasks/T3980-dev-login-faithful-impersonation.md) | 7 | 4 | 1.8 | TODO | [ ] | No way to drive the app AS a real account with real data in Playwright (the X-User-ID/test-login bypass skips session-init -> empty/wrong profile). Add dev/staging-only `POST /api/auth/dev-login {email\ |
