...
| T4360 | [Explicit Orderings: BEGIN IMMEDIATE + Invariant Tests](T4360-explicit-orderings-invariants.md) | WAITING ON USER | local atomicity / ordering — shipped `BEGIN IMMEDIATE` at both action endpoints (race detector proven) + activation invariants as tests + retryable 503 on lock-timeout. `activate_game` left unchanged by design (would deadlock). Awaiting merge. |
