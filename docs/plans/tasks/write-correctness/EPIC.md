...
|----|------|--------|-------|
| T4330 | [Unified Action Client: Serialization + Versioning + 409](T4330-action-client-serialization-conflicts.md) | DONE | concurrency (two tabs/gestures race whole-blob RMW) |
| T4340 | [Canonicalize segments_data at Write Time](T4340-canonicalize-segments-at-write.md) | DONE | data format (dual format → inverted-clip / wrong-recap render) |
| T4350 | [Re-Export Must Re-Transform Carried-Forward Highlights](T4350-reexport-retransform-highlights.md) | DONE | transform correctness — **re-scoped 2026-08-22**: task premise was invalid (described "verbatim drift" endpoint is dead code); real live bug is re-export silently DISCARDING overlay-edited highlights and reseeding fresh auto-detection. Re-tiered M→L. Shipped single-clip (L1 stop-discard + L2 real old→raw→new transform, migration v046), loud reset notice for multi-clip. Design doc `docs/plans/tasks/T4350-design.md`. |
| T4355 | [Multi-Clip Highlight Preservation on Re-Export](T4355-multiclip-highlight-preservation.md) | DONE | transform correctness — follow-up filed 2026-08-22 when T4350 was scoped single-clip-first. Shipped: per-region clip attribution derived at transform time from OLD concat offsets, additive `transition` key on the existing snapshot blob (no migration), old→raw→new generalized to N clips, drop+flag on reorder/removal. Design doc `docs/plans/tasks/T4355-design.md`. Merged to master `7c0273e1`. |
| T4360 | [Explicit Orderings: BEGIN IMMEDIATE + Invariant Tests](T4360-explicit-orderings-invariants.md) | DONE | local atomicity / ordering — shipped `BEGIN IMMEDIATE` at both action endpoints (race detector proven) + activation invariants as tests + retryable 503 on lock-timeout. `activate_game` left unchanged by design (would deadlock). Merged to master. |
...
- [x] Two concurrent tabs/gestures cannot silently lose an edit (409 + retry UX instead)
- [x] `segments_data` has one on-disk format; readers don't defensively canonicalize
- [x] Re-export never silently discards or mis-times a user's edited highlights — carried
...
- [x] Action-endpoint RMW atomicity is explicit (BEGIN IMMEDIATE) with a race-detector test
- [x] Backend import check + full backend tests green after each task
