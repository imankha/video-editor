...
- [ ] Before/after `[PROFILE]` lines captured for `GET /quests/progress` (attribution-first).
- [ ] `/progress` opens user.sqlite at most once per request.
- [ ] Above-baseline server time for `/progress` measurably reduced (target: within ~150 ms of the ~200 ms baseline on warm cache).
