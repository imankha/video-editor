...
| T2627 | [Optimize Modal Ingest](T2627-optimize-modal-ingest.md) | TODO |
| T2628 | [Ingest Timeout & Retry](T2628-modal-ingest-timeout-retry.md) | TODO |
...
- T2627 optimizes ingest performance: hash-during-download + parallel multipart upload. Reduces 3GB Veo import from ~5.5 min to ~2 min.
- T2628 adds timeout/retry/cancellation strategy: reduces Modal function timeout to 2x expected time, adds per-attempt caller-side timeout and progress stall detection, returns structured error codes so the UI can show appropriate guidance.
- T2630 is the UI — only starts after T2628 is complete. Adds "Paste Link" option to GameDetailsModal with per-platform help content, including exhaustion error states.
