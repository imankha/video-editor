# T7070: Classical Signal Bake-off (motion vectors / diff / flow + camera compensation)
...
### Related Tasks
- Depends on: T7060 (testbed, labels, metrics). Reuse its `Signal` base, metrics, and report unchanged.
- Blocks: T7080 (reads this task's feature cache)
...
- **Cost curve**: CPU wall-clock per game-hour per signal at each sampling rate, single-threaded and multiprocess. This table feeds T7080's recipe choice and T5460's cost gate.
- Keep everything CPU-only in this task — no Modal, no torch requirement (RAFT quality-reference is optional and may be deferred to T7080's GPU runner).
...
- [ ] Feature cache written for all 8 games and documented (T7080 consumes it)
