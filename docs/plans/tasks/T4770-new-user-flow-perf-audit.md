...
- In parallel, mark **user-perceived milestones on the same clock**: emit `console.warn('[PERF] <milestone> @<ms>')` (or `performance.mark`) at the moments a *user* judges — "screen appeared", "games visible", "video started playing", "spinner gone". `console.warn` (not `log`) so the dev console keeps it. These marks are what let you split delays *from the user's perspective*, which is the whole point.
- Also pull **`PerformanceNavigationTiming` / `PerformanceResourceTiming`** and any `PROFILING_ENABLED` server logs to cross-check the HAR clock.
