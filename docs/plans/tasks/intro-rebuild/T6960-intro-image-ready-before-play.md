...
**2026-08-13 round 2 (live localhost repro, Playwright-instrumented):** the gate worked on a fast link; the user's persistent failures were the PHOTO itself - a 2.2MB PNG (upload had a fully-OPAQUE alpha channel, kept lossless by design-for-T5200) re-downloaded behind a fresh presigned URL every play (per-request signatures defeat the browser cache), blowing the 2.5s cap so the gate degraded to photoless. Fixed: (1) presign_intro_image memoizes presigns ~10min/key (all three URL surfaces switched - card API, playback payload, share payload), (2) _reencode flattens fully-opaque alpha to JPEG (real transparency still PNG), (3) cap raised 2500->8000ms (frontend + edge mirror). Backend 71 green (one pre-existing db-locked flake vs the live dev server), frontend 72 green. NOTE for tester: existing stored photos stay PNG until re-uploaded; the presign cache still makes them cache-hit after the first load.
