...
| 3b | **Photo framing = reposition + zoom, stored as a normalised focal point + zoom** on the card, adjusted by dragging the live card preview. NOT a fixed crop rectangle. | Cards render at **both** 9:16 and 16:9 (reels are either), and a fixed 9:16 crop cannot produce a 16:9 card — it would force the user to crop the same photo twice. A normalised focal point + zoom adapts to any output aspect from one setting, and dragging the real card preview is WYSIWYG. |
