# T04 — Remove false export-required loading warning
...
This file contains the task’s full product brief. Its adjacent `T04.html` is an equivalent single-file visual handoff with screenshots and report mockups embedded. Choose one format; do not read both. Markdown images use the included assets folder; the schematic below is inline text.
...
No prerequisite task. Distinguish unresolved, loading, ready, dirty and failed media state. Use the existing confirmed job/revision state, not defaults that imply dirty work.
...
Use a dedicated neutral loading state. Changing only the warning text cannot fix an incorrect dirty-state predicate.

## Work to perform

- Trace entry into Spotlight immediately after successful framing.
- Suppress export-required state until media/revision readiness is resolved.
- Show real dirty/unexported warning only for confirmed unmet requirements; retain actionable media-load failure.

## Proposed replacement copy

- Loading your exported highlight…
- Your framing changes need an export before Spotlight can use them. — only if confirmed
- Couldn’t load your exported highlight. Try again.
...
```text
Add spotlight after export
        ↓ state unresolved
Loading your exported highlight…
        ├─ ready → select/preview spotlight
        ├─ confirmed dirty → explain actual export requirement
        └─ load failed → Retry (no implied paid render)
```
...
- [ ] No export-required message flashes during successful post-export loading.
- [ ] Real unexported changes still show the correct action after readiness is known.
- [ ] Loading failure offers retry without implying another paid render is necessary.

## Practical validation

- Delay media/revision responses separately and reverse their order.
- Test actual dirty state, load failure and subsequent successful retry.
- Verify loading → ready transition in narrow and wide layouts.

## Out of scope

No new costs, repeated paid export, or blanket suppression of genuine errors.
