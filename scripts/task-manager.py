...
function buildBugCard(bug, env, group, isPrimary) {
  ...
  topHtml += '<span class="bug-id" title="Click to copy" onclick="event.stopPropagation();copyText(\'' + bug.id + bugPrefix + '\',this)">#' + bug.id + bugPrefix + '</span>';
  ...
  card.querySelector('.bug-kickoff-btn').onclick = async (e) => {
    ...
    try {
      ...
      prompt += '## Bug ' + b.id + kickoffPrefix + ': ' + (b.description || '').substring(0, 80) + '\n\n';
      ...
      if (relatedBugs.length > 0) {
        ...
        relatedBugs.forEach(r => {
          prompt += '- Bug ' + r.id + kickoffPrefix + ': ' + r.label.replace('_', ' ') + '. ' + r.reason + '\n';
        });
      }
  ...
  card.querySelector('.bug-resolve-btn').onclick = async (e) => {
    ...
    if (!confirm('Resolve' + (total > 1 ? ' all ' + total + ' bugs in this group' : ' bug ' + bug.id + resolvePrefix) + '?')) return;
...
async function updateBugStatusRaw(bugId, env, updates) {
  try {
    ...
    showToast('Bug ' + bugId + (env === 'prod' ? 'p' : 's') + ' updated');
    loadBugs();
    return true;
  } catch(e) {
    ...
    groups.forEach(group => {
      ...
      if (hasRelated) {
        ...
        group.related.forEach(r => {
          ...
          relDiv.innerHTML =
            '<span class="bug-id" title="Click to copy" onclick="event.stopPropagation();copyText(\'' + r.bug.id + relPrefix + '\',this)">#' + r.bug.id + relPrefix + '</span> ' +
