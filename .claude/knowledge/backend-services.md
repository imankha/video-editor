...
- **Backfill:** `app/services/credit_backfill.py` `run_backfill(user_ids=None, apply=False, limit=None, offset=0)` — merges each user's local `user.sqlite` ledger against the R2 copy. NULL-`reference_id` rows (no natural key) are matched via longest-common-prefix content-signature comparison across the two copies, NOT SQLite rowid (rowids collide across independent per-file sequences and would silently drop a local-only grant as a false "duplicate"). Re-derive accounts for open PG reservations so re-running backfill can't over-credit a user with an in-flight export. Chunked via `limit`/`offset` for large user counts; full unchunked runs persist their result via `save_report()`/`load_last_report()`.
