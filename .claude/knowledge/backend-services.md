...
session), and `_run_startup_recovery`'s `recover_orphaned_jobs` (export_worker.py, flipped `async def`
-> plain `def`, no `await` in its body) is now offloaded via `run_in_context` instead of running
directly on whichever loop the fire-and-forget recovery task lands on. See "Where blocking work is
offloaded (T6200)" below for the full detail. Guard: `tests/test_t9135_cold_boot_offload.py`.)
updated: 2026-09-08 (T9130: the Publish page-load stall was the T6200 fingerprint reproduced by NINE
independent un-offloaded handlers, not one shared cause (T9120 measured each holding the single loop
separately; burst wall == the SUM of serial times). Fix = the same two moves T6200/T6240/T8020 use:
flip a whole-body-synchronous handler `async def` -> plain `def` (anyio threadpool, contextvars
propagated) when it has NO `await`, else offload its blocking prefix via `run_in_context`. Flipped to
plain `def`: `admin_me` (admin.py), `get_warmup_urls` (storage.py), `collections_summary` +
`get_collection_intro_batch` (collections.py), `list_unacknowledged_exports` (exports.py),
`rank_confidence` (rank.py), `list_intro_cards` (intro_cards.py), `record_achievement` (quests.py),
`list_pending_uploads` (games_upload.py). Offloaded/restructured: `bootstrap`'s `_read_profile_misc`
now runs via `run_in_context` as a THIRD concurrent gather leg instead of trailing projects+games on
the loop; `list_projects`'s fire-and-forget poster warm now sweeps the per-project
`file_exists_in_r2` existence checks in ONE `run_in_context` call instead of one blocking boto3 HEAD
per project on the loop (the biggest latent amplifier, ~88ms/project); `poster_warmer.py`'s two
dedup double-check HEADs (`file_exists_in_r2` :113, `r2_head_object_global` :182) now offload too.
**Direct-caller landmine hit again (as T8020 warned):** flipping `async def`->`def` broke tests
calling the handler via `asyncio.run(...)`/`await` -- updated the direct callers in
test_collections_summary / test_reel_ranking / test_t5195_intro_cards / test_t5230_intro_compliance /
test_achievement_post_returns_progress to plain sync calls (TestClient callers and test_t6030's
iscoroutine-guarded `_run` helper needed no change). Guard: tests/test_t9130_publish_burst_concurrency.py
(ticker-witness overlap property, counterfactual-proven). The two COLD-PATH offenders outside the
burst -- `auth.py:198 init_session` (inline `user_session_init`, 2145ms cold) and
`session_init.py:408` startup recovery -- were split into follow-up T9135, NOT folded in.)
...
updated: 2026-09-22 (T11010: admin upload attempt/success pairs made grain-coherent + the funnel bar chart stopped LOOKING like it counted events. **Three read-side defects, one new event.** (1) `clip_upload_attempted` was registered in `FLOW_EVENTS` by T8370 and reserved by T8380 for a future client "Add Clip" beacon, but **emitted by nothing** -- `admin.py`'s clip "tried" half summed `clip_save_attempted` (real, annotate flow) plus that dead name, so every direct-upload account read "0 tried / N succeeded". (2) The Games pair mixed grains: `game_created` (games.py `create_game`) is once per GAME, `game_upload_succeeded` (games_upload.py `finalize_upload`) is once per VIDEO FILE, so a multi-angle game rendered "1 / 5" -- success exceeding attempt. **Fix for both: ONE new `game_upload_attempted` FLOW_EVENT (`daily_col: None` -> no `daily_counters` column, NO migration) emitted alongside `clip_upload_attempted` from the SAME seam** -- `prepare_upload`, immediately after the EXISTS dedup early-return, keyed on the request's explicit `kind`. **Landmine for anyone touching this: `kind` is the ONLY legitimate discriminator between a game video and a clip video.** A duration/file-size threshold was explicitly rejected by the user-facing decision (a 15-second game video and a several-minute highlight source both exist); `kind` is client-stated but validated to a closed set with a loud 400, and it is what `upload_object_key`/`finalize_upload` already route on. **Emission boundaries are deliberate and load-bearing:** NOT on the EXISTS dedup branch (no bytes move, so no `_succeeded` could ever follow -- counting it re-creates the very asymmetry this fixes), NOT on a validation refusal (that is `record_upload_failure`, not an attempt), but YES on a resume (a genuine second attempt at the same file). `admin.py` `list_users` exposes `game_tried_count`/`game_succeeded_count` (both per-FILE) for the cell; `game_created_count` stays exposed as the per-GAME funnel/sort dimension and the two are never mixed -- `_SORT_COLUMNS` is UNCHANGED (no new column was added, same reason T8220 needed no entry; see the T8230 landmine above for when it IS required). **Known limitation, not a bug:** `game_upload_attempted` has no history and is not backfillable (synthesising it from `game_created` or `game_upload_succeeded` would be fabricated data), so pre-existing accounts read "0 / N" until they upload again -- self-correcting. (3) The FUNNEL was ALWAYS `COUNT(DISTINCT a.user_id)` on both endpoints (`list_users`'s `funnel_totals` and `analytics_funnel`) -- it never counted events. What made it read as event counts: `FunnelChart.jsx` looked up keys `framing_opened`/`framing_exported` while **the backend derives every funnel key from the event LABEL** (`label.lower().replace(' ','_')`), and those events are labelled "Focus Opened"/"Focus Exported" -- so both rows rendered a hardcoded 0 even while the user table showed accounts sitting AT Focus Opened. **That label->key derivation is the recurring trap in this file** (T7930 hit it renaming "Annotation Done"): a chart stage key is NEVER the event name, and renaming a FLOW_EVENTS label silently re-keys its funnel row. `clip_uploaded` had been in `FUNNEL_STEPS` since T8370 but was never listed in `STAGES`, so the whole direct-upload flow was invisible. And the % column was step-over-step conversion on NON-NESTED steps (more users watch the Annotate video than save a clip; more export an overlay than export from Focus), yielding 114%/138%/700% -- now share of signed-up, bounded and comparable, with a caption stating the bars are distinct users. **This does NOT close T7465** (Sankey journey graph), which names the same symptom; it is the cheap honesty fix on the existing chart. `PlatformBreakdown.jsx` `ACTION_LABELS` gained the three previously-unlabelled events ADDITIVELY (an action with no label there simply does not render -- the T8220 landmine); existing labels were left alone so T8220's guard stands. **Dedup-branch asymmetry (review finding, load-bearing): the EXISTS branch emits for CLIPS but not for GAMES.** The two kinds reach their success event by different routes -- `game_upload_succeeded` fires only in `finalize_upload`, which needs an `upload_session_id` the EXISTS branch never returns, so a game dedup hit can never be followed by a success and counting it would be pure attempt inflation. `clip_uploaded` fires from a DIFFERENT endpoint (`POST /api/clips/upload`, per newly-created `raw_clips` row), and **the R2 clip object is keyed per USER (`upload_object_key(kind, hash, user_id)`) while `raw_clips` is per PROFILE** -- so the same file dedup-hitting at prepare and then landing a row under a second profile (multi-athlete households, a shipped feature), or re-uploaded after its row was deleted, is a real "0 tried / 1 succeeded" if the branch stays silent. Same-profile re-posts are idempotent in the batch (`already_existing`), so that case books an attempt with no success -- inflation in the SAFE direction, which keeps `tried >= succeeded` readable. Two further known over-counts, documented at the call site rather than fixed, both safe-direction: a lost ack on an executed prepare retries via `apiFetchWithNetworkRetry` into the resume branch (two attempts for one user attempt), and an unaffordable upload books an attempt because affordability is REPORTED (`can_afford`) rather than refused -- the client aborts, so no success can follow. The emission uses `await run_in_context(record_milestone, ...)` (`utils/offload.py`), the same offload idiom `_write_upload_failure` uses 190 lines up -- positional-args-only, copies contextvars so the impersonation guard resolves in the worker thread. **`UserDetailPanel.jsx` had the IDENTICAL grain bug** and was fixed in the same pass: it paired `game_created` (per GAME) against `game_upload_succeeded` (per FILE) on the panel an admin opens by clicking a `UserTable` row, so the two surfaces disagreed for the same user AND a negative `gap` silently hid the red deficit chip. It now reads `game_upload_attempted` and renders attempted/succeeded in the SAME ORDER as the table (it was backwards -- harmless while the table said "tried"/"succeeded", ambiguous once T11010 removed the words). `UserTable.jsx`'s own `FunnelSummary` strip needed the same percentage fix as `FunnelChart` (same screen, same `funnelTotals`, two different percentage semantics). The response key `game_upload_succeeded_count` was DROPPED in favour of `game_succeeded_count` (one datum, one name); its only readers were `tests/test_admin.py`. Guards: `tests/test_t11010_upload_attempt_pairs.py` (both kinds emit at prepare, `kind` decides not file size, GAME dedup hit emits nothing while CLIP dedup hit DOES, rejected request emits nothing, registration/no-daily-col/not-a-funnel-step), `FunnelChart.test.jsx` (label-derived keys, Clip Uploaded stage, no-row-exceeds-100% over the real prod shape), `UserTable.test.jsx` (bare `N / M`, cell ignores per-GAME `game_created_count`), `UserDetailPanel.test.jsx` (per-GAME count never feeds the per-file pair). **Test landmine hit during review: `games_upload.py` does `from app.storage import r2_head_object_global`, so monkeypatching `app.storage.r2_head_object_global` is a NO-OP and the test makes a real boto3 HEAD against R2** -- patch `app.routers.games_upload.r2_head_object_global` instead. Three of the new tests shipped with the wrong target and passed by accident.)
...
- **Welcome grant (T11170, moved from T8120):** the 80-credit "welcome" grant that (with the 8-credit `new_account_bonus`) makes the advertised 88 free credits lives in **`storage_credits.grant_welcome_credits`** (amount `storage_credits.WELCOME_CREDITS = 80`), NOT in `credit_ledger` and NOT in the quest system. Called once per session from `session_init._init_slow_path`; idempotent (fixed key `questbank:{user_id}` + remainder computed from prior grants). **FROZEN ledger strings:** it still writes source `quest_upfront` / key prefix `questbank:`, and `credit_ledger._granted_quest_chain_credits` still sums `('quest_reward','quest_upfront')` to compute the already-paid remainder — renaming any of these makes every existing account look like it has 0 granted and pays the 80 again (comments pin this in both files). The T9760 admin backfill endpoint was DELETED (G2): the per-login JIT grant reaches every user on their next session.
...
- **T8650 (epic 4/6): admin revenue AGGREGATES read the ledger, `total_spent_cents` is a display
  ...
  FOLLOWS the dashboard filters (user decision 2026-09-24, round 2):** with NO real segment filter
  (`has_real_filter` = the `_build_segment_filter` parts before appending `excl`) the headline is
  the platform grand total (`_ledger_revenue_total`, deleted payers included, test excluded); with
  a real filter (origin/date/paying/...) it is `SUM(pay.revenue_cents)` over the SAME
  `user_segments{seg_join}{seg_where}` population the Signups number uses — a deleted payer has no
  segment row so can never match a filter (correct, no remainder in pulse). **Grouped views expose
  an unattributed remainder**: channels + cohorts responses carry `unattributed_revenue_cents =
  scoped grand total - Σ bucket revenue`, so attributed + unattributed == grand total (deleted or
  out-of-window payers surfaced, never dropped). The grand total is scoped by
  `_grouped_view_grand_total(cur, exclude_test, origin)`: whole-platform when unfiltered, but scoped
  to a non-default `origin` filter so `/cohorts?origin=X` reconciles WITHIN origin X instead of
  absorbing every other origin's revenue (channels takes no origin filter, so it stays whole-platform).
...
**Disabled tutorial steps must not become unsatisfiable gates (T9410, 2026-09-11).** T8690 turned tutorial videos off and hid the four `watch_*_tutorial` step CTAs from the checklist (frontend `TUTORIAL_VIDEOS_ENABLED = false` in `config/questDefinitions.jsx`), but left those steps in `quest_config` step_ids while `claim_reward` still gated all of them. A **genuinely fresh** account (never deleted/reregistered) can never fire `watched_annotate_tutorial` (no CTA), so quest_1 never enters the completed/claimed set and the 34p/35p idempotency shortcut above does NOT cover it — the claim 400'd ("step 'watch_annotate_tutorial' is incomplete") under a panel showing 5/5. This was a SECOND, distinct route to the same message from the delete/reregister one. **Fix:** `quests._check_all_steps` now treats the four watch steps as satisfied while videos are off (`tutorials_off or 'watched_*_tutorial' in achieved`, reading `quest_config.TUTORIAL_VIDEOS_ENABLED` **live** — module import, so tests/ops can toggle). Because `_check_all_steps` is the single derivation `/progress`, `/claim-reward`, AND `/bootstrap` all share, a displayed-complete quest and a rejected claim can no longer disagree — and the guarantee does NOT depend on the frontend filter and this flag staying in lockstep. This is a forward completion rule keyed on a flag (same shape as the `add_clip`/`rate_clip` OR-backfills), not a defensive fixup. **Landmine:** the backend `TUTORIAL_VIDEOS_ENABLED` (`quest_config.py`) and the frontend one MUST be flipped together — flipping one re-arms the gate on only one side. When ON, the OR drops out and the real achievement gate returns (proven by `test_tutorial_quest_steps.py`, which now forces the flag True). **Error copy:** `claim_reward` now raises structured `detail={code, quest_id, step_id, message}`; the frontend (`QuestPanel.handleClaimReward`) names the HUMAN task via `STEP_TITLES[step_id]` and keeps the raw step id as a console diagnostic only (N39/N40; T9560 owns final vocabulary). Tests: `test_t9410_tutorial_step_gating.py`.
