...
**Truth-based version gate (Tbug40p — replaced the T5070 sha-mismatch heuristic):** the decision is `serverBuild > clientBuild` (strictly), where both are a **monotonic, orderable build number** = `git rev-list --count HEAD` (identical for FE and BE built from the same commit — that's why it's comparable; git shas are NOT orderable and are kept only for logs/bug-reports). Client bakes its own number into the bundle via the `__APP_BUILD__` vite define (`vite.config.js`), immutable for the loaded page — it is truth, not a latched observation. Backend advertises `X-App-Build` on every response (`AppVersionHeaderMiddleware`, outermost so it survives 401s/preflight) + `GET /api/version` `build`; value = `APP_BUILD` build-arg (Dockerfile ARG/ENV; deploy-backend.yml + deploy_production.sh compute `git rev-list --count HEAD` — **both need `fetch-depth: 0`** or a shallow clone counts 1; `version.py` falls back to `0` locally, and `0 > N` is always false so a dev/misconfigured server never false-gates). Client SSOT is `appVersion.checkServerVersion(X-App-Build)`: the passive `sessionInit.js` interceptor calls it on EVERY /api response (zero extra requests); `pwaUpdate.js` adds an on-load + resume (visibilitychange/pageshow) poll plus a 30s visible-tab poll (T9360; 30s check gap, 25s probe gap since T10940). Why this kills the bug class: a running build can't re-detect ITSELF (no ack needed — the loaded bundle IS the acknowledgement), and a straggler backend on a LOWER number is `<= clientBuild` so it never gates — **no debounce/candidate machinery needed** (all deleted: `bootVersion`/`candidateVersion`/`acknowledgedVersion`/sessionStorage ack).

**T10940 landmine (2026-09-21, prod incident: stale UI after a deploy, plain reload still stale, only a hard refresh escaped):** `virtual:pwa-register` -> workbox-window `register({immediate:false})` waits for the window `load` event, so `pwaUpdate`'s `registration` is NULL for seconds after boot while the on-load `GET /api/version` answers in ~200ms. After a deploy the server is ahead on EVERY load of the stale precached shell, so the on-load probe used to run against `null`, answer "no bundle", and stamp `lastProbeAt` with the FULL 5-min cooldown -- the client then sat on the old bundle for 5 min even though the browser had staged the new SW as `waiting`; a plain reload just repeated the race. Fix: `probeForWaitingBundle(getRegistration, registrationSettled)` AWAITS the registration (settled by `onRegisteredSW`/`onRegisterError`, or immediately when `!('serviceWorker' in navigator)`), bounded by `REGISTRATION_WAIT_MS` 15s; and there is ONE probe cooldown, `PROBE_MIN_GAP_MS` = 25s, deliberately < pwaUpdate's 30s visible poll so every tick that finds the server ahead may probe (T9310 Gap C's two-tier `{hasBundle, stillInstalling}` cooldown is deleted -- the probe returns a boolean). A "no" is always a snapshot: `deploy_production.sh` ships the backend BEFORE the Pages bundle. Never answer the probe against a null registration, never lock a "no" out for minutes. Proof lives ONLY in `e2e/T6230-update-gate-real-sw.spec.js` case 5 (`npm run test:e2e:sw-gate`, real two-build SW fixture, NOT in CI): red on the old code (no reload in 90s), green in ~31s (30s of which is the unauthenticated cold-boot guard). Harness gotcha fixed alongside: `vite build` defaults to `--mode production` and picks up a developer's gitignored `.env.production` (`VITE_API_BASE=https://api.reelballers.com`), which CORS-blocks the fixture's own `/api/version` -- `buildTwoBundles` now builds with `VITE_API_BASE=''`.
