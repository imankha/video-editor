...
project WITHOUT navigating — the play becomes an editable clip, left for a later
Framing pass; reuses the SAME `onFullscreenUpdateClip(id, { createProject: true })`
seam `clipStage.js`'s NO_PROJECT `createActions` already documents as the "create"
outcome, just not previously exposed on this row). Render: while `!selectedRegion
?.autoProjectId`, the row is [Edit Play] on its own line + [Frame Now]/[Frame Later]
on a second line below; once `autoProjectId` exists, it collapses back to the single
existing [Edit Play] + stage-CTA-label button (`getClipStage`'s FOCUS/SPOTLIGHT/
FINAL/PUBLISHED label) — the split only ever applies to the create decision, never
to an already-created clip. `Frame Later` gets `motion-safe:animate-pulse` when
`region.rating === CLIP_NUDGE_RATING` (imported from `playProgress.js` — the SAME
constant `PlayProgressBadges`' NUDGE state keys off, T10410; a fresh Reviewer
pass caught the first draft comparing against an inlined `5` instead). New
...
first. Fresh Reviewer: 2 MAJOR (the throb-threshold magic number above; an em
dash in the new `FRAME_LATER_HINT` shipped tooltip copy) + 6 MINOR, 0
BLOCKING — both MAJOR fixed, plus hardened the double-create guard with a
synchronous `frameCreateInFlightRef` (T9830/T10240 convention: two buttons now
share one create seam, so a ref beats state-only gating even though no real
click path exploited it) and renamed the collapsed stage button's testid
`annotate-frame-clip-cta` -> `annotate-stage-cta` (it no longer covers the
create case). Tests: `AnnotateModeView.frameClip.test.jsx` (rewritten for the
3-button split + throb + mid-flight-disabled assertions). Prior:)
