...
- **Branded outro (T3950):** "Made with Reel Ballers" (~1.75s dark card) attached via two non-invasive paths:
  ...
  - **T5240 — animated card (2026-07-22):** the download-burn card is now ANIMATED entirely inside `_build_outro_card`'s `filter_complex` (still encoded ONCE + cached, so animation is free at export). Motion: (1) WHITE-FLASH entrance (`fade=t=in:color=white`, `OUTRO_FLASH_IN`) — replaces the old black fade-up, sharing the player-intro's flash vocabulary (T5210); (2) logo slides up + fades in (`format=rgba`+`fade=alpha` on a `-loop 1 -t OUTRO_DURATION` logo stream so the alpha ramp has frames; overlay `y=` ease-out-cubic expr); (3) staggered captions ("Made with" `OUTRO_MADE_IN_ST` then URL `OUTRO_URL_IN_ST`, drawtext `alpha='if(...)'` ramps). `_CARD_VERSION` bumped `v2-logo`→`v3-animated` so stale cached cards rebuild. **Landmine:** expression values are wrapped in single quotes in the filtergraph — do NOT also backslash-escape their commas (leaks `\` into the expr evaluator). Concat/probe-match/non-fatal/cache contract unchanged. Tests: `test_t5240_animated_outro.py` (flash entrance, logo reveal, caption stagger via luma-over-time) + `test_t3950_branded_outro.py` (unchanged, still green).
