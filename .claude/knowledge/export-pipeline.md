...
- **Branded outro (T3950):** "Made with Reel Ballers" end-card is composited at PLAYBACK TIME, NOT burned into the exported MP4. Wired in the frontend shared viewers only: `BrandedEndCard.jsx` (React) rendered by `SharedVideoOverlay` (on `MediaPlayer.onEnded`) and `SharedCollectionView` (on `CollectionPlayer.onEnded`); inline DOM overlay in the edge-function share page (`functions/shared/[token].js`, `v.ended` listener). Gate: `BRANDED_OUTRO_ENABLED` constant (`src/frontend/src/constants/brandedOutro.js`, default true). Prop-gated to PUBLIC/SHARED surfaces -- never shown in the editor, ranker, or owner's My Reels player. No backend changes, no migration, no re-encode: every existing reel gets the card for free on next view.
...
- ~~**T3950**~~ REWORKED 2026-07-12 (design pivot): burn-in approach REJECTED by user. Now playback-composited via `BrandedEndCard` on public/shared viewers. No backend changes. See the Data-flow "Branded outro" bullet above.
