...
updated: 2026-08-05 (T5210 player intro card render engine); 2026-08-04 (T5225 overlay text layer); 2026-08-04 (T5180 rich text engine); 2026-07-22 (T5240 animated branded outro; T4200/T4210/T4230/T4240/T4280 bug sweep; T3950 branded outro)
---
...
- **Player intro card (T5210, 2026-08-05):** an animated intro card PREPENDED at serve time (T5220 wires egress) — the mirror of the branded outro in the prepend direction. `services/player_intro.py`:
  - `build_intro_card(card, field_values, image_path, info, out_path) -> bool` is a PURE function (no DB, no R2, no job rows — T5220 resolves the card row, the profile fact VALUES, the effective focal, and downloads the R2 image/cutout to a local path). `prepend_intro_card(card_path, main_path, out_path)` does the probe-matched `-c copy` concat with the **card FIRST**, re-encode fallback, then `_validate_concat`. **NON-FATAL always** (epic dec 9): every failure returns False/None and logs; it NEVER raises into a download/share/export.
