...
- **Branded outro (T3950):** "Made with Reel Ballers" (~1.75s dark card) attached via two non-invasive paths:
...
- **Share posters / og:image (`services/poster.py`, T4890 -> T5090/T5180):** one frame stored as a JPEG in R2 so share links unfurl with a real image. Selection heuristic `extract_clearest_frame_jpeg` = JPEG-encode a few samples, keep the LARGEST (detail encodes bigger; motion blur/defocus compress away). **Poster policy is per artifact type:**
  - **Reels** (`generate_and_store_poster(user_id, filename, clip_segments)`): clearest frame in the **FIRST HALF of the first slow-mo section** on the FINAL (stretched, concatenated) timeline; **no slow-mo / no segment data -> plain first frame** (NOT whole-clip sampling). `first_slowmo_section(clips)` walks ordered `(segments_data, source_duration)` per working clip reusing `highlight_transform` (`canonicalize_segments_data` for the dual boundary format, `get_segment_speed`, `get_trim_range`; output = source/speed), accumulating per-clip offsets so MULTI-CLIP reels find the first slow-mo across the whole concatenation, respecting `trimRange`. `extract_clearest_frame_jpeg(..., window=(start,end))` restricts sampling to that absolute-time span (the first half). Segments are read via `read_clip_segments_for_project(cursor, pid)` / `load_project_clip_segments(pid)` (latest-version subquery + `sort_order`, SAME as multi-clip render). Threaded from BOTH overlay finalize paths (`_finalize_overlay_export` loads before the write txn; `export_final` reuses its open cursor). **Backfill** (`backfill_posters(force=True)`) reconstructs segments per final video from the DB (project -> latest working_clips -> segments_data) so backfill == live publish; archived/unreconstructable -> first frame, logged at info, NO fabricated slow-mo region. Poster stored frozen on `final_videos.poster_filename`; **failure NEVER fails the export** (best effort, returns None).
  - **Game/teammate recaps** (T5180): **whole-clip clearest frame** (recaps are stitched, no per-segment slow-mo -> reel policy does NOT apply, selection helper unchanged). `ensure_recap_poster(recap_key, poster_key)` generates from `recaps/{game_id}.mp4` -> caches at deterministic `recaps/posters/{game_id}.jpg` (generate-on-first-request, reuse-if-cached via HEAD, overwrite-safe, never raises). Served via token-gated `GET /api/shared/teammate/{token}/poster.jpg` (`shares.py`, mirrors `_serve_poster_jpeg`; 24h cache; 404 -> edge branded-card fallback). `upload_bytes_to_r2_global(key, ...)` writes to a full env-prefixed key under the SHARER's prefix from the unauthenticated request. Teammate GET returns `poster_url` (stable proxy path) when a recap exists; teammate edge fn emits og:image via it.
