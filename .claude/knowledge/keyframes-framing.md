...
to both `<FocusScreen>` and `<OverlayScreen>`) could satisfy a readiness poll (`!!ref.current`)
against the WRONG instance on tick zero: `handlePublish` calls the poller synchronously in the same
tick as `setEditorMode('overlay')`, before React swaps Focus out, so the ref still holds Focus's
still-mounted FRAMING button and `triggerExport()` posts to `/api/export/render` (framing) instead
of `/api/export/render-overlay`. Fix = SPLIT the ref per mode (`focusExportButtonRef`,
`overlayExportButtonRef`), NOT gate on `editorMode === OVERLAY` — `setEditorMode` is a synchronous
Zustand `set()`, so `editorMode` is already `'overlay'` on the tick-zero check while a shared ref is
still Focus's handle (the store state LAGS the actual ref/mount by a tick, so the compound gate fires
on the framing button exactly as before). New `scheduleOverlayPublishExport` (scheduleExportWhenReady.js)
binds the poll to `overlayExportButtonRef` specifically + an `onAbandon` callback (fires once if the
