...
set "CID="
for /f "usebackq delims=" %%i in (`npx --yes @devcontainers/cli up --workspace-folder . --config "%CONFIG%" ^| node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const l=s.trim().split('\n').reverse().find(x=>x.trim().startsWith('{'))||'{}';let o={};try{o=JSON.parse(l)}catch(e){}process.stdout.write(o.containerId||'')})"`) do set "CID=%%i"

if not defined CID (
  echo [claude-docker] could not resolve container id; using devcontainer exec fallback...
  call npx --yes @devcontainers/cli exec --workspace-folder . --config "%CONFIG%" claude %*
  goto :eof
)
